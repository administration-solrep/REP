...
public class FEV541HomologationSha512Operation {
	...
	public void doUpdate(CoreSession session) {
		...
		try {
			...
			List<String> questionsIdsList = QueryUtils.doQueryForIds(session,
			...
			counterThread = new Thread(new Runnable() {
				...
				public void run() {
					...
					while (!Thread.currentThread().isInterrupted()) {
						...
						LOG.info(String.format("Mise à jour SHA-512: %d/%d questions traitées (%.2f/s.)",
								currentCounter, total, speed));
						try {
			...
			LOG.error("Erreur de mise à jour des questions", ce);
	...
	public void doUpdateLot(List<String> questionsIds, AtomicInteger counter,
			ConfigService configService, QuestionConnexeService questionConnexeService) {
		...
		try {
			...
			try {
				...
				questionsDocumentModelList = lotSession.getDocuments(questionsIds, null);
	...
	public boolean doUpdateQuestion(CoreSession session, QuestionConnexeService service, DocumentModel questionDocumentModel) {
