...
@Operation(id = ExtraireQuestionsOperation.ID, category = CaseConstants.CASE_MANAGEMENT_OPERATION_CATEGORY, label = "ExtraireQuestions", description = "Permet d'extraire les questions au format xml")
public class ExtraireQuestionsOperation {
	...
	public void run() throws Exception {
		try {
			LOGGER.info(session, ReponsesLogEnumImpl.INIT_OPERATION_EXTR_QUEST_TEC, "-------------------------------------------------------------------------------");
			...
			if ("encours".equals(statut)) {
				...
				LOGGER.error(session, ReponsesLogEnumImpl.FAIL_PROCESS_OPERATION_EXTR_QUEST_TEC, "Le paramètre 'statut' doit être égal à 'encours' ou 'closes'");
				return;
			}
	
			if (!QuestionTypesEnum.isValueInEnum(type)) {
				StringBuilder message = new StringBuilder("Le paramètre facultatif type doit être égal à l'une des valeurs suivantes : ");
			...
			if (origine == null) {
				...
				LOGGER.error(session, ReponsesLogEnumImpl.FAIL_PROCESS_OPERATION_EXTR_QUEST_TEC, "Le paramètre facultatif 'origine' doit être égal à 'AN' ou 'SENAT'");			
			}
		} finally {
			LOGGER.info(session, ReponsesLogEnumImpl.END_OPERATION_EXTR_QUEST_TEC, "-------------------------------------------------------------------------------");
		}
	}
	
	private void fireEvent(final EventProducer producer, final Map<String, Serializable> eventProperties) throws ClientException {
