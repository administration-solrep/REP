...
public class VerserDossiersPourEliminationOperation {
    ...
    private static final String QUERY_DOSSIER_CANDIDAT = "select dossier_reponse.id as id from DOSSIER_REPONSE where IDDOCUMENTQUESTION in (select question.id from QUESTION where DATEPUBLICATIONJO < ?) and LISTEELIMINATION is null";
	
	private static final String QUERY_COUNT_DOSSIER_CANDIDAT = "select count(dossier_reponse.id) from DOSSIER_REPONSE where IDDOCUMENTQUESTION in (select question.id from QUESTION where DATEPUBLICATIONJO < ?) and LISTEELIMINATION is null";
    ...
    public void run() throws Exception {
        ...
        if (!checkParameters(nbDossiers)) {
        	LOGGER.warn(session, ReponsesLogEnumImpl.FAIL_OPERATION_VERSER_DOSSIER_TEC, "Les paramètres reçus ne sont pas valides");
        	return;
        }
        ...
        LOGGER.info(session, ReponsesLogEnumImpl.PROCESS_OPERATION_VERSER_DOSSIER_TEC,  countDossiers + " dossiers candidats trouvés");
        if (countDossiers > 0) {
        	LOGGER.info(session, ReponsesLogEnumImpl.PROCESS_OPERATION_VERSER_DOSSIER_TEC, "Nombre de dossiers qui seront traités : " + nbDossiers);
    ...
    private boolean checkParameters(int nbDossiers) throws ClientException {
    	
    	if (nbDossiers <= 0) {
    		LOGGER.error(session, ReponsesLogEnumImpl.FAIL_OPERATION_VERSER_DOSSIER_TEC, "Le nombre de dossiers à verser ne peut pas être négatif ou nul");
    		return false;
    	}
    ...
    private int getNbDossiersCandidat(Object[] params) throws ClientException {
    ...
    private void verserDossiers(Object[] params) throws ClientException {
        ...
        List<DocumentModel> dossierDocs = new ArrayList<DocumentModel>();
        List<Dossier> dossierErreurs = new ArrayList<Dossier>();
        ...
        for (Long offset = (long) 0; offset < nbDossiers; offset += limit) {
            ...
            LOGGER.info(session, ReponsesLogEnumImpl.PROCESS_OPERATION_VERSER_DOSSIER_TEC, "Récupération dossiers de " + offset + " à " + borne);
            ...
            LOGGER.info(session, ReponsesLogEnumImpl.PROCESS_OPERATION_VERSER_DOSSIER_TEC, "Fin traitement dossiers de " + offset + " à " + borne);
        }
        
        if (!dossierErreurs.isEmpty()) {
        	LOGGER.warn(session, ReponsesLogEnumImpl.PROCESS_OPERATION_VERSER_DOSSIER_TEC, "Dossiers n'ayant pu être ajoutés à la liste d'élimination : " + dossierErreurs.size());
        	for (Dossier dos : dossierErreurs) {
        		Question question = dos.getQuestion(session);
        		LOGGER.warn(session, ReponsesLogEnumImpl.PROCESS_OPERATION_VERSER_DOSSIER_TEC, "Dossier " + question.getSourceNumeroQuestion() + " - législature : " + question.getLegislatureQuestion());
        	}
        }        
    }
