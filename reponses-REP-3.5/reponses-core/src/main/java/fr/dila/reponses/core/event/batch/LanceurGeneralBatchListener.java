...
public class LanceurGeneralBatchListener extends AbstractBatchEventListener {
	...
	protected void processEvent(final CoreSession session, final Event event) throws ClientException {
		...
		try {
			if (checkMigrationRunning(session)) {
				...
				LOGGER.info(session, STLogEnumImpl.END_B_LANCEUR_GENERAL_TEC,
						"Pas de lanceur général : une migration est considérée en cours !");
				return;
			}
			...
			for (WeeklyBatchEnum weeklyBatch : WeeklyBatchEnum.values()) {
				if (ReponsesEventConstant.EXTRACTION_QUESTIONS_BATCH_EVENT.equals(weeklyBatch.getEventName())) {
					// Cas particulier du batch d'extraction des questions : pas de paramètre, il est déclenché tous les vendredis
		...
		try {
			suiviBatchService.createBatchResultFor(batchLoggerModel, "Exécution du lanceur général", endTime
					- startTime);
		} catch (Exception e) {
	...
	private void executeExtractionQuestionsBatch(final CoreSession session, final boolean open,
			final EventProducer eventProducer, final Map<String, Serializable> eventProperties) throws ClientException {
		if (open) {
