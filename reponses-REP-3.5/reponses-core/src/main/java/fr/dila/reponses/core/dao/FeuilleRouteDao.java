package fr.dila.reponses.core.dao;
...
public class FeuilleRouteDao {
	 ...
	 */
	private FeuilleRouteCriteria	criteria;
	 ...
	 */
	public FeuilleRouteDao(CoreSession session, FeuilleRouteCriteria criteria) throws ClientException {
	...
	protected void build(final CoreSession session) throws ClientException {
		...
		StringBuilder sb = new StringBuilder("SELECT DISTINCT r.").append(STSchemaConstant.ECM_SCHEMA_PREFIX)
				.append(":").append(STSchemaConstant.ECM_UUID_PROPERTY).append(" AS id, r.")
				.append(STSchemaConstant.ECM_SCHEMA_PREFIX).append(":")
				.append(STSchemaConstant.ECM_CURRENT_LIFE_CYCLE_STATE).append(", r.")
				.append(STSchemaConstant.DUBLINCORE_SCHEMA_PREFIX).append(":")
				.append(STSchemaConstant.DUBLINCORE_TITLE_PROPERTY).append(", r.")
				.append(STSchemaConstant.FEUILLE_ROUTE_SCHEMA_PREFIX).append(":")
				.append(STSchemaConstant.FEUILLE_ROUTE_MINISTERE_PROPERTY).append(", r.")
				.append(STSchemaConstant.DUBLINCORE_SCHEMA_PREFIX).append(":")
				.append(STSchemaConstant.DUBLINCORE_CREATOR_PROPERTY).append(", r.")
				.append(STSchemaConstant.DUBLINCORE_SCHEMA_PREFIX).append(":")
				.append(STSchemaConstant.DUBLINCORE_MODIFIED_PROPERTY).append(", r.")
				.append(STSchemaConstant.FEUILLE_ROUTE_SCHEMA_PREFIX).append(":")
				.append(STSchemaConstant.FEUILLE_ROUTE_DEMANDE_VALIDATION_PROPERTY).append(" FROM ")
				.append(STConstant.FEUILLE_ROUTE_DOCUMENT_TYPE).append(" AS r ");

		boolean joinRouteStep = StringUtils.isNotBlank(criteria.getRoutingTaskType())
				|| StringUtils.isNotBlank(criteria.getDistributionMailboxId()) || criteria.getDeadline() != null
				|| criteria.getAutomaticValidation() != null || criteria.getObligatoireSGG() != null
				|| criteria.getObligatoireMinistere() != null;

		if (joinRouteStep) {
			sb.append(", ").append(STConstant.ROUTE_STEP_DOCUMENT_TYPE).append(" AS s ");
		}

		sb.append(" WHERE r.").append(STSchemaConstant.ECM_SCHEMA_PREFIX).append(":")
				.append(STSchemaConstant.ECM_PARENT_ID_PROPERTY).append(" = ? ");
		paramList.add(feuilleRouteModelFolder.getId());

		if (criteria.isCheckReadPermission()) {
			final StringBuilder sb2 = new StringBuilder("r.").append(STSchemaConstant.FEUILLE_ROUTE_SCHEMA_PREFIX)
					.append(":").append(STSchemaConstant.FEUILLE_ROUTE_MINISTERE_PROPERTY);
		...
		if (StringUtils.isNotBlank(criteria.getCurrentLifeCycleState())) {
			sb.append(" AND r.").append(STSchemaConstant.ECM_SCHEMA_PREFIX).append(":")
		...
		if (StringUtils.isNotBlank(criteria.getCreationUtilisateur())) {
			sb.append(" AND r.").append(STSchemaConstant.DUBLINCORE_SCHEMA_PREFIX).append(":")
		...
		if (criteria.isFeuilleRouteDefaut()) {
			sb.append(" AND r.").append(STSchemaConstant.FEUILLE_ROUTE_SCHEMA_PREFIX).append(":")
		...
		if (StringUtils.isNotBlank(criteria.getMinistere())) {
			...
			sb.append(" OR r.").append(STSchemaConstant.FEUILLE_ROUTE_SCHEMA_PREFIX).append(":")
					.append(STSchemaConstant.FEUILLE_ROUTE_MINISTERE_PROPERTY).append(" ='' )");
		}
		...
		if (StringUtils.isNotBlank(criteria.getIntitule())) {
			...
			// TODO à enlever lorsque l'injection de fdr aura été corrigé
			sb.append(" OR r.").append(STSchemaConstant.DUBLINCORE_SCHEMA_PREFIX).append(":")
					.append(STSchemaConstant.DUBLINCORE_TITLE_PROPERTY).append(" ='' )");
		}

		if (joinRouteStep) {
			sb.append(" AND r.").append(STSchemaConstant.ECM_SCHEMA_PREFIX).append(":")
					.append(STSchemaConstant.ECM_UUID_PROPERTY).append(" = s.")
					.append(STSchemaConstant.ROUTING_TASK_SCHEMA_PREFIX).append(":")
					.append(STSchemaConstant.ROUTING_TASK_DOCUMENT_ROUTE_ID_PROPERTY);

			if (StringUtils.isNotBlank(criteria.getRoutingTaskType())) {
				sb.append(" AND s.").append(STSchemaConstant.ROUTING_TASK_SCHEMA_PREFIX).append(":")
			...
			if (StringUtils.isNotBlank(criteria.getDistributionMailboxId())) {
				sb.append(" AND s.").append(STSchemaConstant.ROUTING_TASK_MAILBOX_ID_XPATH).append(" = ? ");
