...
public class RouteStepChangedListener implements PostCommitEventListener {
	...
	public void handleEvent(EventBundle events) throws ClientException {
		Set<DocumentModel> listeDocModifies = new HashSet<DocumentModel>();
		if ((events.containsEventName(STEventConstant.AFTER_SUBSTITUTION_FEUILLE_ROUTE))
			...
			for (Event evt : events) {
				if ((evt.getName().equals(STEventConstant.AFTER_SUBSTITUTION_FEUILLE_ROUTE))
					...
					if (ctx instanceof DocumentEventContext) {
						...
						if (listeDocModifies.add(modelDoc)) {
							...
							if (fdrDoc != null) {
								...
								String label = getLabelFromRunningSteps(runningSteps, feuilleRouteService, session,
										fdrDoc.getId());
	...
	private String getLabelFromRunningSteps(List<DocumentModel> runningSteps, FeuilleRouteService feuilleRouteService,
			CoreSession session, String idFdr) throws ClientException {
