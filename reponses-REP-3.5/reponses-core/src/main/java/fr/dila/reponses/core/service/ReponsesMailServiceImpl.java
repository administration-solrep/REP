...
public class ReponsesMailServiceImpl extends STMailServiceImpl implements ReponsesMailService {
	...
	public void sendMailUserPasswordCreation(final CoreSession session, final String userId, final String password)
		...
		String message;
		if (user.isPermanent()) {
			subject = stParamService.getParametreValue(session,
					STParametreConstant.NOTIFICATION_MAIL_CREATION_UTILISATEUR_OBJET);
			message = stParamService.getParametreValue(session,
		...
		try {
			LOGGER.info(session, STLogEnumImpl.SEND_MAIL_TEC, "Envoi d'un mél ayant pour objet : " + subject
			...
			LOGGER.error(session, STLogEnumImpl.FAIL_SEND_MAIL_TEC,
					"Erreur lors de l'envoi d'un mél ayant pour objet : " + subject + " à l'adresse suivante : "
							+ email, e);
	...
	public void sendMailAfterSignatureError(final CoreSession session, final DocumentModel dossierDoc)
		...
		try {
			...
			LOGGER.error(session, STLogEnumImpl.FAIL_SEND_MAIL_TEC, "Mail suite à l'échec de signature d'un dossier", e);
		}
	}

	@Override
	public void sendMailNoRouteFound(final CoreSession session, final Question question) throws ClientException {
		final STParametreService paramService = STServiceLocator.getSTParametreService();
		...
		try {
			...
			LOGGER.error(session, STLogEnumImpl.FAIL_SEND_MAIL_TEC,
					"Mail suite à l'échec de la recherche d'un modèle de feuille de route pour un dossier", e);
		}
	}
	 ...
	 */
	private List<String> getAdminFonctionnelMailReceiver() throws ClientException {
		final List<STUser> users = STServiceLocator.getProfileService().getUsersFromBaseFunction(
				STBaseFunctionConstant.ADMIN_FONCTIONNEL_EMAIL_RECEIVER);

		// Récuperation des adresses e-mail des administrateurs fonctionnels
		final List<String> destinataires = new ArrayList<String>();
	...
	@Override
	public void sendMailAfterStateChangedQuestion(final CoreSession session, final DocumentModel dossierDoc,
			final String nouvelEtat) throws ClientException {
		final STParametreService paramService = STServiceLocator.getSTParametreService();
		...
		final List<String> listIdDossiers = new ArrayList<String>();
		...
		final List<DocumentModel> runningSteps = ReponsesServiceLocator.getFeuilleRouteService().getRunningSteps(
				session, fdrDoc.getId());
		for (final DocumentModel runningStep : runningSteps) {
			// Récupération des utilisateurs du poste
			final String mailboxId = runningStep.getAdapter(STRouteStep.class).getDistributionMailboxId();
			...
			try {
				...
				if (posteNodeBDC == null) {
					LOGGER.warn(session, STLogEnumImpl.FAIL_GET_POSTE_TEC, "Inexistence du poste BDC de l'entite "
							+ idMinistere);
				} else {
		...
		if (VocabularyConstants.ETAT_QUESTION_SIGNALEE.equals(nouvelEtat)) {
			...
			objet = paramService
					.getParametreValue(session, ReponsesParametreConstant.OBJET_MAIL_DOSSIER_RENOUVELLEMENT);
		...
		// Envoi du mail
		try {
			STServiceLocator.getSTMailService().sendHtmlMailToUserListWithLinkToDossiers(session, userList, objet,
					texte, listIdDossiers);
	...
	@Override
	public void sendDailyRetiredMail(final CoreSession session) throws ClientException {
		final STParametreService paramService = STServiceLocator.getSTParametreService();
		String objet = "";
		String texte = "";
		...
		if (!questionListe.isEmpty()) {
			LOGGER.info(session, ReponsesLogEnumImpl.END_B_DAILYMAIL_RET_TEC, "Nombre de questions retirées :"
					+ questionListe.size());
			...
			try {
				...
				if (!questionCaduqueListe.isEmpty()) {
					objet = paramService.getParametreValue(session,
							ReponsesParametreConstant.OBJET_DAILY_RETIRED_CADUQUE);
					texte = paramService.getParametreValue(session,
							ReponsesParametreConstant.TEXTE_DAILY_RETIRED_CADUQUE);
					processForEachRetiredState(session, objet, texte, questionCaduqueListe);
				}
			} catch (final Exception e) {
				LOGGER.error(session, STLogEnumImpl.FAIL_SEND_MAIL_TEC,
						"Mail de notification des retraits de type CADUQUE", e);
			}
			...
			try {
				...
				if (!questionRetireeListe.isEmpty()) {
					objet = paramService.getParametreValue(session,
							ReponsesParametreConstant.OBJET_DAILY_RETIRED_RETIREE);
					texte = paramService.getParametreValue(session,
							ReponsesParametreConstant.TEXTE_DAILY_RETIRED_RETIREE);
					processForEachRetiredState(session, objet, texte, questionRetireeListe);
				}
			} catch (final Exception e) {
				LOGGER.error(session, STLogEnumImpl.FAIL_SEND_MAIL_TEC,
						"Mail de notification des retraits de type RETIREE", e);
			}
			...
			try {
				...
				LOGGER.error(session, STLogEnumImpl.FAIL_SEND_MAIL_TEC,
						"Mail de notification des retraits de type CLOTURE_AUTRE", e);
			}
		} else {
	 ...
	 * @throws ClientException
	 */
	private List<Question> getLastDayRetiredQuestion(final CoreSession session) throws ClientException {
		final Calendar dateExpiration = Calendar.getInstance();
		dateExpiration.add(Calendar.DAY_OF_MONTH, -1);
		final StringBuffer selectRetiredQuestion = new StringBuffer().append(
				"select q.ecm:uuid AS id from Question AS q WHERE q.qu:dateRetraitQuestion > ? ").append(
				" OR q.qu:dateCaduciteQuestion > ? OR q.qu:dateClotureQuestion > ? ");
		final List<DocumentModel> questionRetirees = QueryUtils.doUnrestrictedUFNXQLQueryAndFetchForDocuments(session,
				DossierConstants.DOSSIER_DOCUMENT_TYPE, selectRetiredQuestion.toString(), new Object[] {
						dateExpiration, dateExpiration, dateExpiration });

		// Récupération des questions retirées
		final List<Question> questionListe = new ArrayList<Question>();
	...
	private List<Question> getCaduqueListeQuestion(final List<Question> questionsRetirees, CoreSession session) {
		final List<Question> resultatListe = new ArrayList<Question>();
	...
	private void processForEachRetiredState(final CoreSession session, final String objetMail, final String texteMail,
			final List<Question> listeQuestion) throws ClientException {
		final STMailService mailService = STServiceLocator.getSTMailService();
		final STPostesService posteService = STServiceLocator.getSTPostesService();
		final MailboxPosteService mailboxPosteService = SSServiceLocator.getMailboxPosteService();
		final Map<STUser, List<Question>> userToQuestion = new HashMap<STUser, List<Question>>();
		final List<STUser> userList = new ArrayList<STUser>();
		final String queryLastStep = " SELECT id as id FROM ROUTING_TASK where DOCUMENTROUTEID = ? AND dateFinEtape = ( SELECT MAX(dateFinEtape) FROM ROUTING_TASK where DOCUMENTROUTEID = ? ) ";
		...
		for (final Question question : listeQuestion) {
			...
			try {
				res = QueryUtils.doSqlQuery(session, new String[] { FlexibleQueryMaker.COL_ID }, queryLastStep,
				...
				LOGGER.error(session, SSLogEnumImpl.FAIL_GET_STEP_TEC,
						"Impossible de récupérer la dernière étape pour une question retirée", e);
			} finally {
				if (res != null) {
					res.close();
				}
			...
			if (idLastStepBeforeRetired != null) {
				...
				// Récupération des utilisateurs du poste
				final String mailboxId = lastStepBeforeRetired.getAdapter(STRouteStep.class).getDistributionMailboxId();
				final String posteId = mailboxPosteService.getPosteIdFromMailboxId(mailboxId);
				try {
					final PosteNode posteNode = posteService.getPoste(posteId);
					userList.addAll(posteNode.getUserList());
			...
			for (final STUser user : userList) {
				...
				if (listquestionPoste == null) {
					listquestionPoste = new ArrayList<Question>();
				}
		...
		for (final Entry<STUser, List<Question>> entry : userToQuestion.entrySet()) {
			if (!entry.getValue().isEmpty()) {
				...
				final List<Map<String, String>> questionList = new ArrayList<Map<String, String>>();
				...
				final List<STUser> usersListToSendMail = new ArrayList<STUser>();
				usersListToSendMail.add(entry.getKey());

				// Envoi du mail
				mailService.sendTemplateHtmlMailToUserList(session, usersListToSendMail, objetMail, texteMail,
						mailParams);
			}
		}
	}
