...
/**
 * Listener asynchrone post-commit qui enregistre l'Audit log pour l'application Réponses.
 * 
 * @author bby, ARN
 */
public class NotificationAuditEventLogger extends STNotificationAuditEventLogger {
		...
		if (event.getContext() instanceof DocumentEventContext) {
			...
			if (model != null) {
				...
				if (DocumentEventTypes.DOCUMENT_CREATED.equals(eventName)
					...
					} else if (STConstant.FEUILLE_ROUTE_DOCUMENT_TYPE.equals(docType)) {
					...
					if (docCtx.getSourceDocument() != null && docCtx.getSourceDocument().getType() != null
						...
						Long major = (Long) model.getProperty(STSchemaConstant.UID_SCHEMA,
					...
					String comment = "Transfert du dossier à ["
	 ...
	 */
	private void logDossier(Event event, DocumentEventContext context) throws ClientException {
		...
		if (dossier != null) {
			...
			Object[] datas = new Object[] { event.getName(), context.getPrincipal().getName(), origineQuestion,
					nomCompletAuteur, dossier.getNumeroQuestion() };
	 ...
	 * @throws ClientException
	 */
	private void logEvent(Event event, DocumentEventContext dossierCtx) throws ClientException {
			...
			if (allotissement != null) {
				...
				for (String id : idsDossiers) {
					...
					try {
						...
						final DocumentEventContext envContext = new DocumentEventContext(session,
								dossierCtx.getPrincipal(), dossierDocAlloti);
						envContext.setComment(dossierCtx.getComment());
						envContext.setCategory(STEventConstant.CATEGORY_PARAPHEUR);
						AUDIT_LOGGER.logEvent(envContext.newEvent(event.getName()));
					} catch (ClientException e) {
						LOGGER.warn("Erreur de récupération d'id pour dossier alloti du dossier id "
								+ dossierDoc.getId() + " - pas de log dans le journal effectué");
					}
				}
			} else {
				AUDIT_LOGGER.logEvent(event);
	 ...
	 * @param context
	 */
	private void logReponse(Event event, DocumentEventContext context) throws ClientException {
		...
		if (reponse != null) {
			DocumentModel dossierModel = reponseService.getDossierFromReponse(context.getCoreSession(),
					reponse.getDocument());
			...
			if (dossierModel != null) {
				...
				Object[] datas = new Object[] { event.getName(), context.getPrincipal().getName(), origineQuestion,
						nomCompletAuteur, dossier.getNumeroQuestion(), reponse.getIdAuteurReponse() };
