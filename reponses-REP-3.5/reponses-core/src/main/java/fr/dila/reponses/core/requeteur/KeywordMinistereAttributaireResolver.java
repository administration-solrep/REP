...
public class KeywordMinistereAttributaireResolver implements RequeteurFunctionSolver {
     ...
     * 
     */
    // TODO: Gérer les cas d'erreurs avec des exceptions "maison"
    @Override
    public String solve(final CoreSession session) {
        if (session == null || !(session.getPrincipal() instanceof SSPrincipal)) {
            LOGGER.error("Ce solveur a besoin d'avoir une session non nulle et un principal de type SSPrincipal, session = " + session);
        ...
        if (principal.isMemberOf(ReponsesBaseFunctionConstant.CORBEILLE_SGG_READER) || principal.isMemberOf(ReponsesBaseFunctionConstant.ALL_CORBEILLE_READER)) {
            return ALWAYS_TRUE_CLAUSE;
        }
    ...
    protected SSPrincipal getPrincipal(final CoreSession session) {
        ...
        if (env != null && env.containsKey(CONTRIBUTOR)) {
            try {
                ...
                LOGGER.error("Le gestionnaire du mot-clé spécial ministere_attributaire n'a pas pu récupérer le principal de l'utilisateur", e);
            }
        } else {
