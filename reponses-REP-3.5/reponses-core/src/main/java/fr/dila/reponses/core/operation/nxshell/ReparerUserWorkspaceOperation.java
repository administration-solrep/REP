 ...
 * @author bgamard
 */
@Operation(id = ReparerUserWorkspaceOperation.ID, category = CaseConstants.CASE_MANAGEMENT_OPERATION_CATEGORY, label = "ReparerUserWorkspace",
...
public class ReparerUserWorkspaceOperation {
    ...
    public void run() throws Exception {
        ...
        for (Long offset = (long) 0; offset <= count; offset += limit) {
            ...
            final List<DocumentModel> workspacesDoc = QueryUtils.doUFNXQLQueryAndFetchForDocuments(session, "Workspace", query, null, limit, 0);
            ...
            for (final DocumentModel workspaceDoc : workspacesDoc) {
                if (workspaceDoc.getPathAsString().startsWith("/case-management/UserWorkspaces")) {
                    ...
                    for (final DocumentModel childDoc : session.getChildren(workspaceDoc.getRef())) {
                        if (childDoc.getType().equals("SmartFolder") || childDoc.getType().equals(STAlertConstant.ALERT_DOCUMENT_TYPE)) {
