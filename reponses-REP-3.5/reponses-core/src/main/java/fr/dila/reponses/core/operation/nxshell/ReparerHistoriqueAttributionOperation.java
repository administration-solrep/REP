 ...
 * @author bgamard
 */
@Operation(id = ReparerHistoriqueAttributionOperation.ID, category = CaseConstants.CASE_MANAGEMENT_OPERATION_CATEGORY,
        label = "ReparerHistoriqueAttribution", description = "Réparer l'historique d'attributions des dossiers")
public class ReparerHistoriqueAttributionOperation {
    private static final String HISTORIQUE_DOSSIER_TRAITE = "SELECT d.ecm:uuid as id FROM Dossier AS d WHERE d.dos:historiqueDossierTraite = ?";
    ...
    public void run() throws Exception {
        ...
        final Long count = QueryUtils.doCountQuery(session, QueryUtils.ufnxqlToFnxqlQuery(HISTORIQUE_DOSSIER_TRAITE), new Object[] { false });
        ...
        for (Long offset = (long) 0; offset <= count; offset += (long) 1000) {
            ...
            final List<DocumentModel> dossiersDoc = QueryUtils.doUFNXQLQueryAndFetchForDocuments(session, "Dossier", HISTORIQUE_DOSSIER_TRAITE,
                    new Object[] { false });
            ...
            for (final DocumentModel dossierDoc : dossiersDoc) {
                ...
                LOGGER.info("Ajout d'une attribution initiale pour le dossier " + question.getSourceNumeroQuestion() + "; ministère: "
                        + question.getIdMinistereInterroge() + "; date: " + date);

                dossier.addHistoriqueAttribution(session, STVocabularyConstants.FEUILLE_ROUTE_TYPE_CREATION_INSTANCIATION, date,
