...
public class QuestionUpdateListener implements EventListener {
	...
	public void handleEvent(Event event) throws ClientException {
        ...
        ReponsesIndexableDocument reponsesIndexableDocument = question.getDocument().getAdapter(ReponsesIndexableDocument.class);
		...
		if(modif){
			...
			StringBuilder queryDL = new StringBuilder("SELECT * FROM ")
			    ...
			    .append(" = 'todo' ");
			DocumentModelList resultListDL = session.query(queryDL.toString());
			...
			for (DocumentModel dossierLinkDoc : resultListDL) {
				dossierDistributionService.updateDenormalisation(dossierLinkDoc.getAdapter(DossierLink.class), question);
