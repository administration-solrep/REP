 ...
 * @author bgamard
 */
@Operation(id = DenormalisationDatesQuestionOperation.ID, category = CaseConstants.CASE_MANAGEMENT_OPERATION_CATEGORY,
        label = "DenormaliserDatesQuestion", description = "Dénormalise les dates liées aux états de la question")
public class DenormalisationDatesQuestionOperation {
    ...
    public void run() throws Exception {
        ...
        final List<Map<String, String>> idQuestionLot = new ArrayList<Map<String, String>>();
        final STParametreService paramService = STServiceLocator.getSTParametreService();
        final String legislature = paramService.getParametreValue(session, ReponsesParametreConstant.LEGISLATURE_COURANTE);

        log.info("Début opération " + ID);

        log.info("Récupération des questions rappelées");
        final List<DocumentModel> questionsDoc = QueryUtils
        ...
        for (final DocumentModel questionDoc : questionsDoc) {
            ...
            if (lot != null && lot.getIdDossiers().size() == 2) {
                ...
                final List<String> listQuestionLot = new ArrayList<String>();
                ...
                final Map<String, String> idQuestion = new HashMap<String, String>();
    ...
    public Boolean sendMail(final CoreSession session, final List<Map<String, String>> idQuestionLot) throws ClientException {
        ...
        final String content = "Dénormalisation date questions rappelées : liste des id de questions rappelées dans des lots de plus de deux questions.";
