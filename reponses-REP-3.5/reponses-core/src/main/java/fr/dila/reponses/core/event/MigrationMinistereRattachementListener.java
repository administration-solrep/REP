...
public class MigrationMinistereRattachementListener implements PostCommitEventListener {
	...
	protected void handleEvent(final Event event) throws ClientException {
		...
		final Map<String, String> newTimbre = (Map<String, String>) properties
				.get(ReponsesEventConstant.MIGRATION_GVT_NEW_TIMBRE_MAP);
		final String reponsesLoggingId = (String) properties.get(ReponsesEventConstant.MIGRATION_GVT_CURRENT_LOGGING);
		final String newTimbreUnchangedEntity = (String) properties
				.get(ReponsesEventConstant.NEW_TIMBRE_UNCHANGED_ENTITY);
		final String newTimbreDeactivateEntity = (String) properties
				.get(ReponsesEventConstant.NEW_TIMBRE_DEACTIVATE_ENTITY);

		LOGGER.logMigration(ReponsesLogEnumImpl.MIGRATE_MIN_RATT_TEC, "Début de la migration des timbres");
		runMigrationUnrestricted(coreSession, newTimbre, reponsesLoggingId, newTimbreDeactivateEntity,
				newTimbreUnchangedEntity);
		LOGGER.logMigration(ReponsesLogEnumImpl.MIGRATE_MIN_RATT_TEC, "Fin de la migration des timbres");
	}

	private void runMigrationUnrestricted(final CoreSession coreSession, final Map<String, String> newTimbre,
			final String reponsesLoggingId, final String newTimbreDeactivateEntity,
			final String newTimbreUnchangedEntity) throws ClientException {
		...
		new UnrestrictedSessionRunner(coreSession) {
			...
			public void run() throws ClientException {
				...
				for (Entry<String, String> entry : newTimbre.entrySet()) {
					...
					final DocumentModel lineDoc = createLine(reponsesLogging, "-" + currentMin + "-" + newMin);
					...
					try {
						if (!newTimbreDeactivateEntity.equals(newMin) && !newTimbreUnchangedEntity.equals(newMin)) {
							...
							Long closeCount = updateTimbreService.getCloseCountForMinistere(coreSession, currentMin);
							if (closeCount == null) {
							...
							String message = "Migration de l'organigramme Questions Closes: ["
									+ oldMinistere.getLabel() + "] vers [" + newMinistere.getLabel() + "]";
							reponsesLoggingLine.setMessage(message);

							LOGGER.logMigration(ReponsesLogEnumImpl.MIGRATE_MIN_RATT_TEC, message);

							listLog = addLogToLine(reponsesLoggingLine, listLog, session,
									"Migration des ministeres de rattachement Questions Closes");

							migrationService.migrateAllCloseDossierMinistereRattachement(session, currentMin,
			...
			private DocumentModel createLine(final ReponsesLogging reponsesLogging, String pos) throws ClientException {
	...
	private List<String> addLogToLine(ReponsesLoggingLine reponsesLoggingLine, final List<String> listLog,
			final CoreSession session, final String message) throws ClientException {
		listLog.add(message);
		LOGGER.logMigration(ReponsesLogEnumImpl.MIGRATE_MIN_RATT_TEC, message);
		reponsesLoggingLine.setFullLog(listLog);
		reponsesLoggingLine.save(session);
