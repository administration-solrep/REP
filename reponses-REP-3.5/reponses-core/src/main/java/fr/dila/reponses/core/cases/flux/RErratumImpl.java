...
public class RErratumImpl implements RErratum {
    ...
    public RErratumImpl(Map<String,Serializable> erratumMap) {
        if (erratumMap != null) {
            if (erratumMap.get(DossierConstants.DOSSIER_ERRATUM_DATE_PUBLICATION_PROPERTY) instanceof GregorianCalendar) {
                this.datePublication = ((GregorianCalendar) erratumMap.get(DossierConstants.DOSSIER_ERRATUM_DATE_PUBLICATION_PROPERTY));
            }
            ...
            if (erratumMap.get(DossierConstants.DOSSIER_ERRATUM_TEXTE_CONSOLIDE_PROPERTY) instanceof String) {
                this.texteConsolide = ((String) erratumMap.get(DossierConstants.DOSSIER_ERRATUM_TEXTE_CONSOLIDE_PROPERTY));
            }
        }
    }
