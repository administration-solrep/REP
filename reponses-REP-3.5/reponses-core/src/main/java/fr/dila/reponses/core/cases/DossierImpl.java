...
public class DossierImpl extends STDossierImpl implements Dossier {
	...
	private static final String		QUERY_HISTORIQUES		= "SELECT h.ecm:uuid AS id FROM "
	...
	public String getLastDocumentRoute() {
		return PropertyUtil.getStringProperty(document, DossierConstants.DOSSIER_SCHEMA,
				STSchemaConstant.DOSSIER_LAST_DOCUMENT_ROUTE_PROPERTY);
	}

	@Override
	public void setLastDocumentRoute(String lastDocumentRoute) {
		PropertyUtil.setProperty(document, DossierConstants.DOSSIER_SCHEMA,
	...
	@Override
	public Dossier setDossierProperties(CoreSession session) throws ClientException {
	...
	@Override
	public FondDeDossier createFondDeDossier(CoreSession session) throws ClientException {
		try {
			final String fondDeDossierTitre = "FondDeDossier";

			DocumentModel newFondDeDossierModel = session.createDocumentModel(getDocument().getPath().toString(),
					fondDeDossierTitre, ReponsesFondDeDossierConstants.FOND_DE_DOSSIER_DOCUMENT_TYPE);
	...
	public String getIdMinistereAttributaireCourant() {
		return getStringProperty(DossierConstants.DOSSIER_SCHEMA,
	...
	public void setIdMinistereAttributaireCourant(String idMinistere) {
		setProperty(DossierConstants.DOSSIER_SCHEMA, DossierConstants.DOSSIER_ID_MINISTERE_ATTRIBUTAIRE_COURANT,
				idMinistere);
	}
	...
	public void setIdMinistereAttributairePrecedent(String idMinistere) {
		setProperty(DossierConstants.DOSSIER_SCHEMA, DossierConstants.DOSSIER_ID_MINISTERE_ATTRIBUTAIRE_PRECEDENT,
				idMinistere);
	}
	...
	public Question getQuestion(CoreSession session) {
		try {
			...
			throw new ClientRuntimeException("Failed to retrieve question from dossier", e);
		}
	}
	...
	public Reponse getReponse(CoreSession session) {
		try {
			...
			throw new ClientRuntimeException("Failed to retrieve reponse from dossier", e);
		}
	}

	@Override
	public Boolean isArchivable(CoreSession session) throws ClientException {
	...
	public Boolean getEtapeRedactionAtteinte() {
		return PropertyUtil.getBooleanProperty(document, DossierConstants.DOSSIER_SCHEMA,
				DossierConstants.DOSSIER_ETAPE_REDACTION_ATTEINTE);
	}

	@Override
	public void setEtapeRedactionAtteinte(Boolean isEtapeRedactionAtteinte) {
		setProperty(DossierConstants.DOSSIER_SCHEMA, DossierConstants.DOSSIER_ETAPE_REDACTION_ATTEINTE,
	...
	public void setEtapeSignatureAtteinte(Boolean isEtapeSignatureAtteinte) {
		setProperty(DossierConstants.DOSSIER_SCHEMA, DossierConstants.DOSSIER_ETAPE_SIGNATURE_ATTEINTE,
				isEtapeSignatureAtteinte);
	}

	@Override
	public Long getReaffectionCount() {
		return PropertyUtil.getLongProperty(document, DossierConstants.DOSSIER_SCHEMA,
	...
	public void setIdMinistereReattribution(String idMinistereReattribution) {
		setProperty(DossierConstants.DOSSIER_SCHEMA, DossierConstants.DOSSIER_ID_MINISTERE_REATTRIBUTION,
	...
	@Override
	public Boolean isActive(CoreSession session) throws ClientException {
		if (!hasFeuilleRoute()) {
	...
	public void createQuestion(CoreSession session, Question questionObject, String etatQuestion) {
		try {
			...
			DocumentModel question = session.createDocumentModel(getDocument().getPathAsString(), titreQuestion,
	...
	public DocumentModel createReponse(CoreSession session, Long numeroQuestion, Reponse reponseData) {
		try {
			...
			DocumentModel reponse = session.createDocumentModel(getDocument().getPath().toString(), titreReponse,
			...
			if (reponseData != null) {
				...
				question.setHasReponseInitiee(!StringUtils.isBlank(reponseData.getTexteReponse()));
	...
	public List<HistoriqueAttribution> getHistoriqueAttribution(CoreSession session) {
		...
		List<HistoriqueAttribution> historiques = new ArrayList<HistoriqueAttribution>();
		try {
			List<DocumentModel> historiquesDocs = QueryUtils.doUFNXQLQueryAndFetchForDocuments(session,
					"HistoriqueAttribution", QUERY_HISTORIQUES, new Object[] { this.getQuestionId() });
		...
		Collections.sort(historiques, new Comparator<HistoriqueAttribution>() {
			@Override
			public int compare(HistoriqueAttribution histo1, HistoriqueAttribution histo2) {
	...
	@Override
	public void addHistoriqueAttribution(CoreSession session, String fdrTypeCreationInstanciation, Calendar date,
			String idMinistere) throws ClientException {

		String titreAttribution = "Attribution-" + DateUtil.formatYYYYMMdd(date.getTime()) + "-" + idMinistere;

		DocumentModel historiqueDoc = session.createDocumentModel(getDocument().getPath().toString() + "/Question",
				titreAttribution, ReponsesSchemaConstant.HISTORIQUE_ATTRIBUTION_DOCUMENT_TYPE);
	...
	public Boolean isArbitrated() {
		return PropertyUtil.getBooleanProperty(getDocument(), DossierConstants.DOSSIER_SCHEMA,
	...
	protected List<Signalement> getSignalementsProperty(String schema, String value) {
		ArrayList<Signalement> signalementList = new ArrayList<Signalement>();
		List<Map<String, Serializable>> signalements = PropertyUtil.getMapStringSerializableListProperty(document,
				schema, value);
	...
	protected List<Renouvellement> getRenouvellementsProperty(String schema, String value) {
		...
		List<Map<String, Serializable>> renouvellements = PropertyUtil.getMapStringSerializableListProperty(document,
				schema, value);
