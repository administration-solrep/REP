...
then 
	...
	echo " "
	echo "Exemple : sh ldap-genererLdifEnvironnement.sh 'localhost' 'cn=nuxeo5,ou=Reponses,dc=dila,dc=fr' 'changeme' 'ldapscript.ldif' 'suffixe'"
	exit 1
else 
	echo "Hote LDAP : "$1
	echo "Compte LDAP : "$2
...
invariants=("uid=ws_agriculture,ou=people,ou=Reponses,dc=dila,dc=fr"
	"uid=ws_sword,ou=people,ou=Reponses,dc=dila,dc=fr"
	"uid=ws_interieur_DGCL,ou=people,ou=Reponses,dc=dila,dc=fr"
	"uid=ws_an,ou=people,ou=Reponses,dc=dila,dc=fr"
	"uid=ws_ecologie,ou=people,ou=Reponses,dc=dila,dc=fr"
	"uid=ws_senat,ou=people,ou=Reponses,dc=dila,dc=fr"
	"uid=ws_transport,ou=people,ou=Reponses,dc=dila,dc=fr"
	"uid=ws_dila,ou=people,ou=Reponses,dc=dila,dc=fr"
	"uid=ws_interieur,ou=people,ou=Reponses,dc=dila,dc=fr"
	"uid=hebergementDILA,ou=people,ou=Reponses,dc=dila,dc=fr")
...
admins=($(ldapsearch -x -h ${hote} -D ${compte} -w ${mdp} -b "cn=Administrateur fonctionnel,ou=groups,ou=Reponses,dc=dila,dc=fr" -s sub -LLL uniqueMember | perl -p00e 's/\r?\n //g' | grep uniqueMember | grep ou=people | perl -p00e 's/uniqueMember:(.*)/$1/g'))
...
SAVEIFS=$IFS
IFS=$'\n'
allusers=($(ldapsearch -x -h ${hote} -D ${compte} -w ${mdp} -b "ou=Reponses,dc=dila,dc=fr" -s sub "objectclass=personne" dn -LLL | grep dn | perl -p00e 's/(.*):(.*)/$2/g'))
