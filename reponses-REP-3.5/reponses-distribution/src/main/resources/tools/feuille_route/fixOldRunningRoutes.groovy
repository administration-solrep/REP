...
/**
 * script groovy d'annulation de feuille de route à l'état 'running' sans dossier les utilisant (par exemple cas de l'allotissement)
 * 
 */
class FixUtils {    
    ...
    **/
    public static void cancelFeuilleRoute (final CoreSession session, final String feuilleId) {
        if (feuilleId == null) {
            ...
            STFeuilleRoute feuilleRoute = feuilleDoc.getAdapter(STFeuilleRoute.class);
            print "Mise à l'état 'cancel' de la feuille de route id : " + feuilleId;
            try {
            	feuilleRoute.cancel(session);
            } catch (Exception e) {
            	print "Impossible de passer à l'état cancel la feuille de route d'id : " + feuilleId;
            }
        }
    }
    ...
    public static void printInfo(Long count) {
        if (count == null) {
            ...
            print "Nombre de feuille de route à l'état 'running' sans dossier les utilisant : " + count;
        }
    }
    
    /**
     *
     * Dénombre les feuilles de route à l'état 'running' sans dossier les utilisant
     */
    public static Long countFeuilleRoute (final CoreSession session) {
        String queryCount = "select count(id) from FEUILLE_ROUTE WHERE id in (SELECT id FROM MISC WHERE lifecyclestate = 'running') AND id not in (SELECT lastdocumentroute FROM DOSSIER_REPONSE WHERE lastdocumentroute IS NOT NULL)";
        ...
        try {
            ...
            print "Impossible de récupérer le nombre de feuille de route à l'état 'running' sans dossier les utilisant"
            print exc;
        } finally {
            if (resCount != null) {
                resCount.close();
    ...
    public static boolean fixRoutes(final CoreSession session, Long count) {
        String query = "select id from FEUILLE_ROUTE WHERE id in (SELECT id FROM MISC WHERE lifecyclestate = 'running') AND id not in (SELECT lastdocumentroute FROM DOSSIER_REPONSE WHERE lastdocumentroute IS NOT NULL)";        
        ...
        try {
            ...
            print "Impossible de récupérer les feuille de route à l'état 'running' sans dossier les utilisant"
            print e;
            return false;
        } finally {
            if (res != null) {
    ...
    public static boolean run(final CoreSession session, final String mode) {        
        ...
        if (MODE_FIX.equals(mode)) {
            if (count == null) {
                print "Dénombrement nul des feuilles de routes";
                return false;
            } else {
