...
public class ReportResource extends DefaultObject {
    ...
    protected boolean readParams(Map<String, Object> userParams) throws Exception {
        ...
        // Paramètres liés au modèle de rapport Birt
        List<ReportParameter> params = report.getReportUserParameters();
        if (params.size() > 0) {
        ...
        // Paramètres d'organigramme
        if (data.getString("parameterType") != null && data.getString("parameterValue") != null) {
            userParams.put("parameterType", data.getString("parameterType"));
            userParams.put("parameterValue", data.getString("parameterValue"));
        }
    ...
    public Object pdf() throws Exception {
        ...
        String artifactId = report.getDoc().getTitle() + ".pdf";
        
        report.render(options, userParams);
        return Response.ok(new FileInputStream(reportFile),
                MediaType.APPLICATION_OCTET_STREAM).header("Content-Disposition", "attachment; filename=\"" + artifactId + "\"").build();
    }

    @POST
    @Produces("application/vnd.ms-excel")
    @javax.ws.rs.Path("excel")
    public Object editAndRenderExcel() throws Exception {
        return excel();
    }
    ...
    public Object excel() throws Exception {
        Map<String, Object> userParams = new HashMap<String, Object>();
        if (!readParams(userParams)) {
            return redirect(getPath() + "/editParams?target=excel");
        }
        ...
        String artifactId = report.getDoc().getTitle() + ".xls";
