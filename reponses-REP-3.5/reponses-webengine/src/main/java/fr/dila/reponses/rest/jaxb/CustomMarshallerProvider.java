...
public class CustomMarshallerProvider implements ContextResolver<Marshaller> {
	...
	private static final String PROPERTY_CHARACTER_ESCAPE_HANDLER = "com.sun.xml.bind.marshaller.CharacterEscapeHandler";
	
    Map<Class<?>, Marshaller> marshallerMap;
    Map<Class<?>, JAXBContext> jaxBContextMap;
    ...
    public Marshaller getContext(Class<?> clazz) {
        ...
        if (clazz != ChercherQuestionsResponse.class && clazz != ChercherErrataQuestionsResponse.class && clazz != ChercherReponsesResponse.class && clazz != ChercherErrataReponsesResponse.class) {
        ...
        if (!jaxBContextMap.containsKey(clazz)) {
            try {
                ...
                if(LOGGER.isTraceEnabled()) {
            		LOGGER.trace("CustomMarshallerProvider - added context for "+ clazz.getName() +" to jaxBContextMap");
            	}
