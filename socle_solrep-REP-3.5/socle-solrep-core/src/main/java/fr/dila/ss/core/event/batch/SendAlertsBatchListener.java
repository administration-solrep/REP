...
public class SendAlertsBatchListener extends AbstractBatchEventListener {
    ...
    protected void processEvent(CoreSession session, Event event) throws ClientException {
        ...
        try {
        	suiviBatchService.createBatchResultFor(batchLoggerModel, "Nombre d'alertes qui sont en état d'être exécutées",alerts.size(), endTime-startTime);
        } catch (Exception e) {
        	LOGGER.error(session, STLogEnumImpl.FAIL_CREATE_BATCH_RESULT_TEC,e);
        }
        LOGGER.info(session, STLogEnumImpl.PROCESS_B_SEND_ALERTS_TEC, "Nombre d'alertes qui sont en état d'être exécutées : " + alerts.size());
        ...
        try {
        	suiviBatchService.createBatchResultFor(batchLoggerModel, "Nombre de mail envoyés",mailSentCount, endTime2-startTime2);
        } catch (Exception e) {
        	LOGGER.error(session, STLogEnumImpl.FAIL_CREATE_BATCH_RESULT_TEC,e);
        }
