...
public class DossierLinkValidatorBatchListener extends AbstractBatchEventListener {
	...
	protected void processEvent(CoreSession session, Event event) throws ClientException {
		...
		try {
			...
			ValidateDueDossierLinkUnrestricted runner = new ValidateDueDossierLinkUnrestricted(session,
					batchLoggerModel, nbError);
			...
			LOGGER.error(session, STLogEnumImpl.FAIL_PROCESS_BATCH_TEC,
					"Echec lors du batch de validation automatique", exc);
			++errorCount;
		}
