...
public class SSFondDeDossierServiceImpl extends SSTreeServiceImpl implements SSFondDeDossierService {
	...
	public DocumentModel createFileInFolder(CoreSession session, DocumentModel parent, String fileName, Blob content)
		...
		return super.createFileInFolder(session, parent,
				SSFondDeDossierConstants.FOND_DE_DOSSIER_FICHIER_DOCUMENT_TYPE, fileName, content);
	}
	...
	public DocumentModel createBareFondDeDossierFichier(CoreSession session, DocumentModel parent, String filename,
			Blob content) throws SSException {
		return super.createBareFileInFolder(session, parent,
				SSFondDeDossierConstants.FOND_DE_DOSSIER_FICHIER_DOCUMENT_TYPE, filename, content);
	}
	...
	public DocumentModel createBareFondDeDossierFichier(CoreSession session) throws SSException {
		...
		try {
			fondDeDossierFichier = session
					.createDocumentModel(SSFondDeDossierConstants.FOND_DE_DOSSIER_FICHIER_DOCUMENT_TYPE);
	...
	public DocumentModel createFondDeDossierRepertoire(CoreSession session, DocumentModel parentDoc, String docName)
		...
		try {
			...
			repertoireDoc = session.createDocumentModel(parentDoc.getPathAsString(), docName,
					getFondDeDossierRepertoireType());
			...
			if (childFound == null) {
				...
				// save document in session
				repertoireDoc = session.saveDocument(repertoireDoc);
			} else {
