	...
	protected void processEvent(CoreSession session, Event event) throws ClientException {
		...
		try {
			...
			if (userDocList != null) {
				...
				try {
					suiviBatchService.createBatchResultFor(batchLoggerModel, "Nombre d'utilisateurs à supprimer",
							nbUserToDelete, endTime - startTime);
				} catch (Exception e) {
					LOGGER.error(session, STLogEnumImpl.FAIL_CREATE_BATCH_RESULT_TEC, e);
				}
				LOGGER.info(session, SSLogEnumImpl.PROCESS_B_DEL_USERS_TEC, "Nombre d'utilisateurs à supprimer : "
						+ nbUserToDelete);
				...
				for (DocumentModel userDoc : userDocList) {
					...
					try {
						userWorkspaceDoc = userWorkspaceService.getCurrentUserPersonalWorkspace(userDoc.getId(),
				...
				try {
					suiviBatchService.createBatchResultFor(batchLoggerModel, "Nombre d'utilisateurs supprimés",
							nbUserDeleted, endTime2 - startTime2);
				} catch (Exception e) {
					LOGGER.error(session, STLogEnumImpl.FAIL_CREATE_BATCH_RESULT_TEC, e);
				}
			} else {
				try {
					suiviBatchService.createBatchResultFor(batchLoggerModel, "Aucun utilisateur à supprimer",
							nbUserToDelete, endTime - startTime);
				} catch (Exception e) {
					LOGGER.error(session, STLogEnumImpl.FAIL_CREATE_BATCH_RESULT_TEC, e);
				}
