...
public class AttributionDelegate extends AbstractDelegate {
    ...
    public ChercherAttributionsResponse chercherAttributions(ChercherAttributionsRequest request) {
        ...
        JetonServiceDto dto = getJetonDTOForWebservice(
            idProfilUtilisateur,
            jetonRecu,
            webservice,
        ...
        for (DocumentModel documentModel : docList) {
            ...
            if (!isMinistere && qid.getSource() != origineUtilisateur) {
                ...
                response.setMessageErreur(
                    "Erreur : l'origine (AN/SENAT) de la  " +
        ...
        logWebServiceAction(
            ReponsesEventConstant.WEBSERVICE_CHERCHER_ATTRIBUTION_EVENT,
    ...
    public ChercherAttributionsDateResponse chercherAttributionsDate(ChercherAttributionsDateRequest request) {
        ...
        if (!resultsAccess.isEmpty()) {
            return buildAttributionDateError(
                (TraitementStatut) resultsAccess.get(MAP_TRAITEMENT_STATUT),
        ...
        List<AttributionDate> attributionsDate = new ArrayList<>();
        ...
        if (jetonRecu == null) {
            ...
            for (DocumentModel jetonDocDoc : dto.getJetonDocDocList()) {
                ...
                try {
                    ...
                    LOGGER.error(
                        "Can't get document " + jetonDoc.getIdDoc() + " for jeton " + jetonDoc.getDocument().getId(),
                        e
                    );
                    continue;
                }
        ...
        // log dans le journal d'administration
        logWebServiceAction(
            ReponsesEventConstant.WEBSERVICE_CHERCHER_ATTRIBUTION_DATE_EVENT,
    ...
    public ChercherMembresGouvernementResponse chercherMembresGouvernement(ChercherMembresGouvernementRequest request) {
        ...
        for (OrganigrammeNode ministere : ministeres) {
            ...
            if (entite.getDateDebut() != null) {
                mg.setDateDebut(DateUtil.dateToXmlGregorianCalendar(entite.getDateDebut()));
            }

            if (entite.getDateFin() != null) {
                mg.setDateFin(DateUtil.dateToXmlGregorianCalendar(entite.getDateFin()));
            }
        ...
        // log dans le journal d'administration
        logWebServiceAction(
            ReponsesEventConstant.WEBSERVICE_CHERCHER_MEMBRES_GOUVERNEMENT_EVENT,
    ...
    public ChercherLegislaturesResponse chercherLegislatures() {
        ...
        try (Session session = STServiceLocator.getDirectoryService().open(VocabularyConstants.LEGISLATURE)) {
            legislatureModelList = session.query(new HashMap<>());
        } catch (Exception e) {
            response.setMessageErreur(RECUPERATION_INFO_KO);
            response.setStatut(TraitementStatut.KO);
            LOGGER.error(RECUPERATION_INFO_KO, e);
        ...
        try {
            legislatureModelList.forEach(
                legislatureModel ->
                    response
                        .getLegislatures()
                        .add(
                            createLegislature(
                                (String) legislatureModel.getProperty(LEGISLATURE_SCHEMA, LEGISLATURE_ID),
                                (Calendar) legislatureModel.getProperty(LEGISLATURE_SCHEMA, LEGISLATURE_DATEDEBUT),
                                (Calendar) legislatureModel.getProperty(LEGISLATURE_SCHEMA, LEGISLATURE_DATEFIN)
                            )
                        )
            );
        } catch (NuxeoException | NumberFormatException exception) {
            response.setMessageErreur(RECUPERATION_INFO_KO);
            response.setStatut(TraitementStatut.KO);
            LOGGER.error(RECUPERATION_INFO_KO, exception);
            return response;
        }
        ...
        // log dans le journal d'administration
        logWebServiceAction(
            ReponsesEventConstant.WEBSERVICE_CHERCHER_LEGISLATURE_EVENT,
    ...
    private Legislature createLegislature(String id, Calendar dateDebut, Calendar dateFin) {
        Legislature legislature = new Legislature();
        legislature.setId(Integer.parseInt(id));
        legislature.setDateDebut(DateUtil.calendarToXMLGregorianCalendar(dateDebut));
        legislature.setDateFin(DateUtil.calendarToXMLGregorianCalendar(dateFin));
        return legislature;
    }

    // ********************************************************************** Méthodes utilitaires
    // *****************************************************
    private boolean setAttributionDate(
        String idMinistere,
        QuestionId questionId,
        AttributionType attributionType,
        Calendar dateAttribution,
        ...
        if (dateAttribution != null) {
            attribution.setDateAttribution(DateUtil.calendarToXMLGregorianCalendar(dateAttribution));
        }
