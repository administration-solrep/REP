...
public class ReponseDelegate extends AbstractDelegate {
    ...
    public ChercherReponsesResponse chercherReponses(ChercherReponsesRequest request) {
        ...
        List<String> lotsEnvoyes = new ArrayList<>();
        List<ReponseQuestion> resultQuestions = new ArrayList<>();
            ...
            for (DocumentModel documentModel : docList) {
                ...
                if (qid.getSource() != origineUtilisateur) {
                    ...
                    response.setMessageErreur(
                        ...
                        " n'est pas de la même origine que celle de l'utilisateur. \n "
                    );
                    break;
                }
                try {
                    // met de coté le lot auquel la question appartient, pour ne plus le renvoyer
                    UnrestrictedGetDocumentRunner uGet = createUnrestrictedGetDocumentRunner(
                    ...
                    response.setStatut(TraitementStatut.KO);
                    response.setMessageErreur(
                        "Impossible de récupérer la question " + qid.getNumeroQuestion() + " : Dossier introuvable."
                    );
                ...
                try {
                    ...
                    response.setStatut(TraitementStatut.KO);
                    response.setMessageErreur(
                        "Impossible de récupérer la question " + qid.getNumeroQuestion() + " : Question introuvable."
                    );
                    LOGGER.error(session, ReponsesLogEnumImpl.FAIL_GET_QUESTION_TEC, e);
                    break;
                }
        ...
        logWebServiceAction(
            ReponsesEventConstant.WEBSERVICE_CHERCHER_REPONSES_EVENT,
    ...
    public ChercherErrataReponsesResponse chercherErrataReponses(ChercherErrataReponsesRequest request) {
        ...
        if (dto == null) {
            ...
            JetonService jetonService = STServiceLocator.getJetonService();
            Long numeroJetonSuivant = jetonService.getNumeroJetonMaxForWS(
                session,
                origineUtilisateur.toString(),
                webservice
            );
        ...
        List<ErratumReponse> resultQuestions = new ArrayList<>();
        List<String> lotsEnvoyes = new ArrayList<>();
        ...
        if (CollectionUtils.isNotEmpty(docList)) {
            for (DocumentModel documentModel : docList) {
                ...
                try {
                    ...
                    rapport.append(
                        "Erreur lors de l'obtention du dossier de la question : " + qid.getNumeroQuestion() + ". \n "
                    );
        ...
        // log l'action dans le journal d'administration
        logWebServiceAction(
            ReponsesEventConstant.WEBSERVICE_CHERCHER_ERRATA_REPONSES_EVENT,
    ...
    private boolean needToValidateReponse() {
        ...
        try {
            return configService.getBooleanValue(ReponsesConfigConstant.VALIDATE_REPONSE_CONTENT_PARAMETER_NAME);
        } catch (NuxeoException e) {
     ...
     *
     */
    public EnvoyerReponsesResponse envoyerReponses(EnvoyerReponsesRequest request) {
        ...
        if (!hasRight(session, STWebserviceConstant.ENVOYER_REPONSES)) {
            ResultatTraitement resultatTraitement = new ResultatTraitement();
        ...
        Set<ResultatTraitement> setTraitement = new HashSet<>();
        ...
        for (ReponseQuestion reponseQuestion : listReponseQuestions) {
            ...
            if (validateReponseContent) {
                ValidatorResult validResult = EnvoyerReponseValidator
                    .getInstance()
                    .validateReponseContent(reponse.getTexteReponse());
            ...
            if (listQuestions == null || listQuestions.isEmpty()) {
                ...
                resultatTraitement.setMessageErreur(
                    "Erreur : aucune question n'est associée à une des réponses soumises"
                );
                setTraitement.add(resultatTraitement);
                continue;
            }
            ...
            for (QuestionId idQuestion : listQuestions) {
                ...
                try {
                    dlink =
                        getDossierLinkFromDossier(dossier, VocabularyConstants.ROUTING_TASK_TYPE_REDACTION_INTERFACEE);
                } catch (NuxeoException e) {
                ...
                fr.dila.reponses.api.cases.Reponse reponseAdapter = reponseModel.getAdapter(
                    fr.dila.reponses.api.cases.Reponse.class
                );
                ...
                question.setHasReponseInitiee(StringUtils.isNotBlank(reponse.getTexteReponse()));
                try {
                    session.saveDocument(question.getDocument());
                    session.save();
                } catch (NuxeoException e) {
                    resultatTraitement.setStatut(TraitementStatut.KO);
                    resultatTraitement.setMessageErreur(
                        "Impossible d'enregistrer la question " + question.getNumeroQuestion()
                    );
                    setTraitement.add(resultatTraitement);
                    LOGGER.error(session, ReponsesLogEnumImpl.FAIL_SAVE_QUESTION_TEC, question.getDocument(), e);
                    continue;
                ...
                try {
                    ...
                    resultatTraitement.setStatut(TraitementStatut.KO);
                    resultatTraitement.setMessageErreur(
                        "Une erreur est survenue lors de la sauvegarde de la réponse du ministère"
                    );
                    setTraitement.add(resultatTraitement);
                ...
                try {
                    ...
                    resultatTraitement.setStatut(TraitementStatut.KO);
                    resultatTraitement.setMessageErreur(
                        "Une erreur est survenue lors de la validation de l'étape 'pour rédaction interfacée'"
                    );
                    setTraitement.add(resultatTraitement);
                ...
                logWebServiceActionDossier(
                    ReponsesEventConstant.WEBSERVICE_ENVOYER_REPONSES_EVENT,
                    ReponsesEventConstant.WEBSERVICE_ENVOYER_REPONSES_COMMENT,
                    dossier
                );
            }
        }
     ...
     * @return
     */
    public EnvoyerReponseErrataResponse envoyerReponseErrata(EnvoyerReponseErrataRequest request) {
        ...
        if (!hasRight(session, STWebserviceConstant.ENVOYER_REPONSES_ERRATA)) {
            ResultatTraitement resultatTraitement = new ResultatTraitement();
            resultatTraitement.setStatut(TraitementStatut.KO);
            resultatTraitement.setMessageErreur(USER_NON_AUTORISE);
            response.getResultatTraitement().add(resultatTraitement);
        ...
        for (ErratumReponse erratumReponse : erratumList) {
            ...
            List<String> lotsModifies = new ArrayList<>();
            ...
            for (QuestionId questionIdPourErratum : questionsIdPourErratum) {
                ...
                if (dossierQuestion == null) {
                    resultatTraitement.setStatut(TraitementStatut.KO);
                    resultatTraitement.setMessageErreur(
                        "Erreur lors de l'identification du dossier " + questionIdPourErratum.getNumeroQuestion()
                    );
                    response.getResultatTraitement().add(resultatTraitement);
                    continue;
                }
                ...
                List<fr.dila.reponses.api.cases.Reponse> listAppReponses = new ArrayList<>();
                try {
                    if (allotissementService.isAllotit(question, session)) {
                        final UnrestrictedGetDocumentRunner uGet = createUnrestrictedGetDocumentRunner(
                            DossierConstants.DOSSIER_SCHEMA
                        );
                        Allotissement allotissement = allotissementService.getAllotissement(
                            dossierQuestion.getDossierLot(),
                            session
                        );
                    ...
                    resultatTraitement.setStatut(TraitementStatut.KO);
                    resultatTraitement.setMessageErreur(
                        "Erreur à l'erratum de la réponse  " + questionIdPourErratum.getNumeroQuestion()
                    );
                    response.getResultatTraitement().add(resultatTraitement);
                ...
                for (fr.dila.reponses.api.cases.Reponse appReponse : listAppReponses) {
                    if (appReponse == null) {
                        resultatTraitement.setStatut(TraitementStatut.KO);
                        resultatTraitement.setMessageErreur(
                            "Erreur lors de l'identification de la réponse " + questionIdPourErratum.getNumeroQuestion()
                        );
                        response.getResultatTraitement().add(resultatTraitement);
                        continue;
                    }
                    ...
                    if (!appReponse.isPublished()) {
                        resultatTraitement.setStatut(TraitementStatut.KO);
                        resultatTraitement.setMessageErreur(
                            "Impossible d'émettre un erratum à une question n'étant pas répondue"
                        );
                        response.getResultatTraitement().add(resultatTraitement);
                        continue;
                    }
                    ...
                    try {
                        ...
                        resultatTraitement.setStatut(TraitementStatut.KO);
                        resultatTraitement.setMessageErreur(
                            "Erreur à l'erratum de la réponse " +
                            questionIdPourErratum.getNumeroQuestion() +
                            " \n" +
                            e.getMessage()
                        );
                ...
                // log l'action dans le journal d'administration
                logWebServiceActionDossier(
                    ReponsesEventConstant.WEBSERVICE_ENVOYER_ERRATA_REPONSES_EVENT,
                    ReponsesEventConstant.WEBSERVICE_ENVOYER_ERRATA_REPONSES_COMMENT,
                    dossierQuestion
                );
            }
        }
     ...
     *
     */
    private ErratumReponse getErratumReponseFromQuestion(
        fr.dila.reponses.api.cases.Question question,
        List<ErratumReponse> errataReponseAdded,
        String webservice
    ) {
        final AllotissementService allotissementService = ReponsesServiceLocator.getAllotissementService();

        DocumentModel reponseDoc = ReponsesServiceLocator
            .getReponseService()
            .getReponseFromDossier(session, session.getDocument(question.getDossierRef()));
        ...
        if (reponse != null) {
            ...
            if (CollectionUtils.isNotEmpty(erList)) {
                ...
                if (allotissementService.isAllotit(question, session)) {
                    ...
                    Allotissement allotissement = allotissementService.getAllotissement(
                        dossierDirecteur.getDossierLot(),
                        session
                    );
                    if (allotissement != null) {
                        for (String idDossier : allotissement.getIdDossiers()) {
    ...
    private void assignDataToReponse(Reponse reponseIn, fr.dila.reponses.api.cases.Reponse reponseOut) {
        ...
        reponseOut.setIdAuteurReponse(Integer.toString(ministreReponse.getId()));
    }
     ...
     *
     */
    public ReponseQuestion getXsdReponsesFromQuestion(
        ...
        if (appQuestion.isRepondue()) {
            ...
            if (StringUtils.isNotBlank(txtReponse)) {
            ...
            if (fdd != null) {
                ...
                if (parentDocRef != null) {
                    ...
                    for (DocumentModel elementFddModel : fichiersModel) {
                        Blob fileBlob = FileSchemaUtils.getContent(elementFddModel);

                        if (
                            fileBlob == null ||
                            StringUtils.isNotEmpty(fileBlob.getFilename()) ||
                            StringUtils.isNoneEmpty(fileBlob.getMimeType())
                        ) {
                            continue;
                        }
                        ...
                        outfile.setNom(fileBlob.getFilename());
                        ...
                        try {
                            ...
                            outfile.setTailleFichier((int) fileBlob.getLength());
                        } catch (IOException e) {
                            continue;
                        }
    ...
    protected Boolean hasUserRightAndSameOrigineQuestion(
        CoreSession session,
        String webservice,
        ...
        if (
            userGroupList.contains(STWebserviceConstant.PROFIL_AN) && questionId.getSource().equals(QuestionSource.AN)
        ) {
            return true;
        } else if (
