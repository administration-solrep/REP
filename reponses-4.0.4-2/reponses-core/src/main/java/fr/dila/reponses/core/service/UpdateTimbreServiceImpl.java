...
public class UpdateTimbreServiceImpl implements UpdateTimbreService {
     ...
     */
    private static final String QUERY_COUNT_CLOSED_QUESTIONS =
        "SELECT " +
     ...
     * Une question close est une QE dont l'état est 'repondu' et qui n'a plus d'étape en cours ou à venir
     */
    private static final String QUERY_COUNT_CLOSED_QUESTIONS_FOR_MIN =
        "SELECT " +
        DossierConstants.DOSSIER_ID_MINISTERE_RATTACHEMENT_QUESTION +
        " as " +
        COL_MINISTERE +
     ...
     *
     */
    private static final String QUERY_CLOSED_QUESTIONS_FOR_MIN =
        "SELECT q.id FROM dossier_reponse d inner join question q on d." +
        DossierConstants.DOSSIER_DOCUMENT_QUESTION_ID +
        " = q.id WHERE q." +
        DossierConstants.DOSSIER_ID_MINISTERE_RATTACHEMENT_QUESTION +
        " = '%s' AND q." +
     ...
     */
    private static final String QUERY_COUNT_SIGNED_QUESTIONS =
        "SELECT " +
     ...
     * une question signée est une question 'QE' avec signature qui a au moins une etape a venir ou en cours dans sa fdr
     */
    private static final String QUERY_SIGNED_QUESTIONS_FOR_MIN =
        "SELECT d.id as " +
        COL_ID +
        ", 1 as " +
        COL_COUNT +
     ...
     * fdr
     */
    private static final String QUERY_COUNT_MIGRABLE_QUESTIONS =
        "SELECT " +
        DossierConstants.DOSSIER_ID_MINISTERE_ATTRIBUTAIRE_COURANT +
        " as " +
        COL_MINISTERE +
     ...
     * fdr
     */
    private static final String QUERY_MIGRABLE_DOSSIERS_FOR_MIN =
        "SELECT d.id as " +
        COL_ID +
        ", 1 as " +
        COL_COUNT +
     ...
     */
    private static final String QUERY_COUNT_MODELE_FDR =
        "SELECT f.ministere as " +
        COL_MINISTERE +
        ", count(1) as " +
        COL_COUNT +
    ...
    @Override
    public Map<String, Long> getCloseCount(final CoreSession session) {
    ...
    private Map<String, Long> buildResultMapFromSQLQueryResult(
    ...
    @Override
    public Map<String, Long> getSigneCount(final CoreSession session) {
        final String[] params = new String[] { VocabularyConstants.QUESTION_TYPE_QE };
    ...
    public List<DocumentModel> getSignedDossiersForMinistere(final CoreSession session, final String idMinistere) {
        ...
        Map<String, Long> mapResult = buildResultMapFromSQLQueryResult(
            session,
            QUERY_SIGNED_QUESTIONS_FOR_MIN,
            params,
    ...
    @Override
    public Map<String, Long> getMigrableCount(final CoreSession session) {
        final String[] params = new String[] { VocabularyConstants.QUESTION_TYPE_QE };
    ...
    public List<DocumentModel> getMigrableDossiersForMinistere(final CoreSession session, final String idMinistere) {
        ...
        Map<String, Long> mapResult = buildResultMapFromSQLQueryResult(
            session,
            QUERY_MIGRABLE_DOSSIERS_FOR_MIN,
            params,
            new String[] { COL_ID, COL_COUNT }
        );
        return QueryUtils.retrieveDocuments(session, DossierConstants.DOSSIER_DOCUMENT_TYPE, mapResult.keySet());
    ...
    @Override
    public Map<String, Long> getModelFDRCount(final CoreSession session) {
    ...
    @Override
    public Boolean isMigrationEnCours(final CoreSession session) {
        final StringBuilder query = new StringBuilder("SELECT r.ecm:uuid as id FROM ReponsesLogging as r ")
            .append(" WHERE r.replog:")
            .append(ReponsesLoggingConstant.REPONSES_LOGGING_END_DATE)
            .append(" IS NULL ");
        return QueryUtils.doCountQuery(session, QueryUtils.ufnxqlToFnxqlQuery(query.toString())) > 0;
    }

    @Override
    public ReponsesLogging getMigrationEnCours(final CoreSession session) {
        final StringBuilder query = new StringBuilder("SELECT r.ecm:uuid as id FROM ReponsesLogging as r ")
            .append(" WHERE r.replog:")
            .append(ReponsesLoggingConstant.REPONSES_LOGGING_END_DATE)
            .append(" IS NULL ");

        final List<DocumentModel> documentModelList = QueryUtils.doUFNXQLQueryAndFetchForDocuments(
            session,
            "ReponsesLogging",
            query.toString(),
            null
    ...
    @Override
    public String createLogging(
        final CoreSession session,
        final Long previsionnalCount,
        final Long closePrevisionalCount,
        final Map<String, String> timbre,
        final String currentGouvernement,
        final String nextGouvernement
    ) {
        DocumentModel modelDesired = session.createDocumentModel(
            ReponsesLoggingConstant.REPONSES_LOGGING_DOCUMENT_TYPE
        );
        modelDesired.setPathInfo(
            ReponsesLoggingConstant.REPONSES_LOGGING_PATH,
        ...
        reponsesLogging.setMessage(
            "Changement de gouvernement du " + SolonDateConverter.DATETIME_SLASH_MINUTE_COLON.formatNow()
        );
        ...
        for (Entry<String, String> entry : timbre.entrySet()) {
            reponsesLogging.addtimbre(entry.getKey(), entry.getValue());
        }
    ...
    @Override
    public List<ReponsesLogging> getAllReponsesLogging(final CoreSession session) {
        ...
        final List<ReponsesLogging> result = new ArrayList<>();
    ...
    @Override
    public List<ReponsesLoggingLine> getAllReponsesLoggingLine(
        final CoreSession session,
        final String reponsesLoggingId
    ) {
        final List<ReponsesLoggingLine> result = new ArrayList<>();
    ...
    @Override
    public List<ReponsesLoggingLineDetail> getAllReponsesLoggingLineDetail(
        final CoreSession session,
        final String reponsesLoggingLineId
    ) {
        final List<ReponsesLoggingLineDetail> result = new ArrayList<>();
    ...
    @Override
    public Long getCloseCountForMinistere(CoreSession session, String currentMin) {
        final String[] params = new String[] { currentMin, VocabularyConstants.QUESTION_TYPE_QE };
        Map<String, Long> mapResult = buildResultMapFromSQLQueryResult(
            session,
            QUERY_COUNT_CLOSED_QUESTIONS_FOR_MIN,
            params
        );
