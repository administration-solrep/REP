...
public class EliminerDossiersOperation {
    ...
    private static final String QUERY_DOSSIERS_IN_LIST =
        "SELECT d.ecm:uuid as id FROM Dossier as d WHERE d.dos:listeElimination = ?";

    private static final String QUERY_COUNT_DOSSIER_IN_LIST =
        "select count(dossier_reponse.id) from DOSSIER_REPONSE where LISTEELIMINATION = ?";
    ...
    public void run() {
        ...
        if (!checkParameters(idList)) {
            LOGGER.warn(
                session,
                ReponsesLogEnumImpl.FAIL_OPERATION_ELIMINER_DOSSIER_TEC,
        ...
        LOGGER.info(
            session,
            ReponsesLogEnumImpl.PROCESS_OPERATION_ELIMINER_DOSSIER_TEC,
            "Nombre de dossiers à supprimer : " + nbDossiersTotal
        );
        ...
        for (Long offset = (long) 0; offset < nbDossiersTotal; offset += limit) {
            ...
            List<DocumentModel> dossiersDocs = QueryUtils.doUFNXQLQueryAndFetchForDocuments(
                session,
                DossierConstants.DOSSIER_DOCUMENT_TYPE,
                QUERY_DOSSIERS_IN_LIST,
                params.toArray(),
                limit,
                0
            );
    ...
    private boolean checkParameters(String idList) {
        ...
        if (session.exists(refList)) {
            ...
            if (listDoc.getAdapter(ListeElimination.class).isEnCours()) {
                LOGGER.warn(
                    session,
                    ReponsesLogEnumImpl.FAIL_OPERATION_ELIMINER_DOSSIER_TEC,
                    "L'état de la liste (en cours), ne lui permet pas d'être supprimée"
                );
                return false;
            }
        } else {
            LOGGER.warn(
                session,
                ReponsesLogEnumImpl.FAIL_OPERATION_ELIMINER_DOSSIER_TEC,
                "L'uid de la liste transmis en paramètre est incorrect : aucune liste n'a été retrouvée"
            );
            return false;
        }
    ...
    private int getNbDossiersToDelete(String idList) {
        List<String> params = new ArrayList<>();
        params.add(idList);
    ...
    private int deleteDossiers(List<DocumentModel> dossiersDocs, int nbDossiersAlreadyDeleted) {
        ...
        for (DocumentModel doc : dossiersDocs) {
            try {
                ...
                FeuilleRoute route = documentRoutingService.getDocumentRouteForDossier(session, doc.getId());
                if (route != null) {
            ...
            if (nbDossiersDeleted % 20 == 0) {
                ...
                LOGGER.info(
                    session,
                    ReponsesLogEnumImpl.PROCESS_OPERATION_ELIMINER_DOSSIER_TEC,
                    nbDossiersDeleted + " dossiers supprimés"
                );
            }
        }
