 ...
 * @author bgamard, spl
 */
public class ArchivageMailListener extends AbstractBatchEventListener implements StoppableBatch {
    /**
    ...
    protected void processEvent(final CoreSession session, final Event event) {
        ...
        try {
            ...
            if (count > 0) {
                ...
                String parametreObjet = paramService.getParametreValue(
                    session,
                    STParametreConstant.NOTIFICATION_MAIL_DOSSIER_ATTENTE_ELIMINATION_OBJET
                );
                String parametreText = paramService.getParametreValue(
                    session,
                    STParametreConstant.NOTIFICATION_MAIL_DOSSIER_ATTENTE_ELIMINATION_TEXT
                );
                ...
                if (
                    parametreText != null &&
                    !parametreText.isEmpty() &&
                    parametreText.contains("${nombreDossiers}") &&
                    parametreText.contains("${dureeDUA}")
                ) {
                    Map<String, Object> paramMap = new HashMap<>();
                    paramMap.put("nombreDossiers", String.valueOf(count));
                    paramMap.put(
                        "dureeDUA",
                        paramService.getParametreValue(session, STParametreConstant.DELAI_CONSERVATION_DONNEES)
                    );
                    text = StringHelper.renderFreemarker(parametreText, paramMap);
                }
                mailService.sendMailToUserList(users, object, text);
            }
        } catch (Exception e) {
        ...
        try {
            suiviBatchService.createBatchResultFor(
                batchLoggerModel,
                "Nombre de dossiers en attente d'archivage",
                count,
                endTime - startTime
            );
        } catch (Exception e) {
    ...
    public STLogEnum getStoppageCode() {
        return ReponsesLogEnumImpl.CANCEL_B_ARCH_MAIL_TEC;
    }
