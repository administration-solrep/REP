...
public class UpdateQuestionConnexeBatchListener extends AbstractBatchEventListener {
    ...
    protected void processEvent(final CoreSession session, final Event event) {
        if (checkMigrationRunning(session)) {
            ...
            LOGGER.info(
                session,
                STLogEnumImpl.END_B_LANCEUR_GENERAL_TEC,
        ...
        try {
            ...
            LOGGER.info(
                session,
                ReponsesLogEnumImpl.PROCESS_B_UPDATE_Q_CONNEXITE_TEC,
    ...
    private boolean checkMigrationRunning(final CoreSession session) {
