 ...
 */
@Operation(
    id = CloturerDossiersLegislatureOperation.ID,
    category = CaseConstants.CASE_MANAGEMENT_OPERATION_CATEGORY,
    label = "CloturerDossiersLegislature",
    description = "Cloture les dossiers en cours d'une législature donnée"
)
public class CloturerDossiersLegislatureOperation {
     ...
     * Logger.
     */
    private static final Log LOGGER = LogFactory.getLog(CloturerDossiersLegislatureOperation.class);
    /**
     * Mode lecture
     */
    ...
    private static final String QUERY =
        "SELECT q.ecm:uuid AS id FROM Question AS q WHERE q.qu:etatQuestion = 'en cours'  ";
    /**
    ...
    public void run() throws OperationException {
        ...
        if (mode.toUpperCase().equals(MODE_LECTURE)) {
            LOGGER.info(
                "Affichage des dossiers à cloturer pour la législature " +
                legislature +
                " et ayant comme origine " +
                origine
            );
            ...
            final List<DocumentModel> questionsDoc = QueryUtils.doUFNXQLQueryAndFetchForDocuments(
                ...
                "Question",
                query + ORDER_BY_ID,
                new Object[] { origine.toUpperCase(), legislature }
            );
            ...
            LOGGER.info(
                "Début de cloture des questions de la législature " +
                legislature +
                " et ayant comme origine " +
                origine +
                " et avec comme type de cloture " +
                typeCloture +
                ". Le traitement se terminera automatiquement à la date " +
                dateFin +
                " même si toutes les questions n'ont pas été traitées."
            );
            // On regarde combien de questions on a en tout
            final List<DocumentModel> questionsDocTotal = QueryUtils.doUFNXQLQueryAndFetchForDocuments(
                session,
                "Question",
                query,
                new Object[] { origine.toUpperCase(), legislature }
            ...
            Date dateFinDate;
            try {
                dateFinDate = SolonDateConverter.DATETIME_DASH_MINUTE_DASH.parseToDate(dateFin);
            ...
            while (onContinue) {
                final List<DocumentModel> questionsDoc = QueryUtils.doUFNXQLQueryAndFetchForDocuments(
                    session,
                    "Question",
                    query + whereQueryDossiersIncorrects + ORDER_BY_ID,
                    new Object[] { origine.toUpperCase(), legislature },
                    10,
                    0
                );
                ...
                for (final DocumentModel questionDoc : questionsDoc) {
                    ...
                    try {
                        ...
                        LOGGER.error(
                            "Une erreur est survenue lors du retrait du dossier de son lot :" + e.getMessage()
                        );
                        whereQueryDossiersIncorrects =
                            whereQueryDossiersIncorrects +
                            QUERY_WHERE_NUMERO_QUESTION +
    ...
    private boolean checkParameters(
        ...
        if (!mode.toUpperCase().equals(MODE_EXECUTION) && !mode.toUpperCase().equals(MODE_LECTURE)) {
            LOGGER.warn(
                "Le mode d'exécution renseigné est incorrect. Il ne peut prendre que deux valeurs. " +
                MODE_EXECUTION +
                " pour l'exécution de la cloture des dossiers et " +
                MODE_LECTURE +
        ...
        if (!origine.toUpperCase().equals(ORIGINE_AN) && !origine.toUpperCase().equals(ORIGINE_SENAT)) {
            LOGGER.warn(
                "L'origine renseignée est incorrecte. Elle ne peut prendre que deux valeurs. " +
                ORIGINE_AN +
                " pour l'exécution de la cloture des dossiers provenant de l'Assemblée Nationale et " +
                ORIGINE_SENAT +
                " pour la cloture des dossiers provenant du Sénat."
            );
            paramsOK = false;
        }
        ...
        if (
            !typeCloture.toUpperCase().equals(CLOTURE_AUTRE) &&
            !typeCloture.toUpperCase().equals(CLOTURE_CADUQE) &&
            !typeCloture.toUpperCase().equals(CLOTURE_RETIREE)
        ) {
            LOGGER.warn(
                "Le type de cloture est incorrect. Ce paramètre ne peut prendre que trois valeurs :" +
                CLOTURE_AUTRE +
                ", " +
                CLOTURE_CADUQE +
                " ou " +
