...
public class JetonServiceImpl extends fr.dila.st.core.service.JetonServiceImpl implements JetonService {
    ...
    public void createJetonTransmissionsAssemblees(CoreSession session, Question question, String webService) {
        ...
        if (allotissementService.isAllotit(dossier)) {
    ...
    protected boolean proceedHighNumberOfJetonDoc(
        ...
        final List<DocumentModel> dossiersDocResult,
        final Long numeroJeton
    ) {
        final AllotissementService allotissementService = ReponsesServiceLocator.getAllotissementService();
        final Map<String, DocumentModel> mapJetonDossiers = new HashMap<>();
                ...
                if (appQuestion == null) {
                    ...
                    if (allotissementService.isAllotit(appQuestion, session)) {
                        final Allotissement allot = allotissementService.getAllotissement(
                            appQuestion.getDossier(session).getDossierLot(),
                            session
                        );
     ...
     *
     */
    private void gestionDossiersAllotis(
        final CoreSession session,
        final List<DocumentModel> jetonsDocPanier,
        final List<String> dossiersIdsAllotis,
        final Map<String, DocumentModel> mapJetonDossiers,
        final List<DocumentModel> jetonsDocList,
        final List<DocumentModel> dossiersDocResult,
        final Long numeroJeton
    ) {
                ...
                if (questionDoc != null) {
                    ...
                    if (jetonDossierDoc == null) {
                        ...
                        StringBuilder query = new StringBuilder("select * FROM JetonDoc WHERE jtd:id_doc = '")
                            .append(questionDoc.getId())
                            .append("' and jtd:id_owner = '")
                            .append(owner)
                            .append("' and jtd:type_webservice = '")
                            .append(typeWs)
                            .append("' and jtd:id_jeton = '")
                            .append(NUMERO_PANIER)
                        ...
                        if (resultsIds.isEmpty()) {
                            ...
                            query =
                                new StringBuilder("select * FROM JetonDoc WHERE jtd:id_doc = '")
                                    .append(dossierDocLot.getId())
                                    .append("' and jtd:id_owner = '")
                                    .append(owner)
                                    .append("' and jtd:type_webservice = '")
                                    .append(typeWs)
                            ...
                            if (resultsIds.isEmpty()) {
                                ...
                                LOGGER.info(
                                    ...
                                    "Problème avec un dossier de l'allotissement. Dossier id : " +
                                    dossierId +
                                    ". Dossiers ids de l'allotissement : " +
                                    org.apache.commons.lang3.StringUtils.join(dossiersIdsAllotis, ", ")
                                );
                                if (questionDoc != null && jetonDossierDoc == null) {
                                    Question question = questionDoc.getAdapter(Question.class);
                                    LOGGER.info(
                                        session,
                                        ReponsesLogEnumImpl.GET_ATTR_QUEST_TEC,
                                        "Question id : " +
                                        questionDoc.getId() +
                                        ". Origine : " +
                                        question.getOrigineQuestion()
