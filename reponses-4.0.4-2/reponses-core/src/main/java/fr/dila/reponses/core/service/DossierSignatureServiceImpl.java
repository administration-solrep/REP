...
public class DossierSignatureServiceImpl implements DossierSignatureService {
    ...
    @Override
    public Boolean signerDossier(Dossier dossier, CoreSession session) {
        try {
            ...
            SignatureExResponse response = d2sServiceCaller.signatureEx(
                requestId,
                dataToSign,
                transactionId,
    ...
    @Override
    public Boolean verifierDossier(Dossier dossier, CoreSession session) {
        try {
            final ConfigService configService = STServiceLocator.getConfigService();
            ...
            if (data == null) {
                LOGGER.warn(
                    "DVS: can not verify : data is null (no TexteReponse) numero question : " +
            ...
            VerifySignatureExResponse response = dvsServiceCaller.verifySignatureEx(
                requestId,
                data,
                signature,
                transactionId
