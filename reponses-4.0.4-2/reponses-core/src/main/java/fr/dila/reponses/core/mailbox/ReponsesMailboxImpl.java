...
public class ReponsesMailboxImpl extends MailboxImpl implements ReponsesMailbox {
    ...
    public List<PreComptage> getPreComptages() {
        List<String> pre = PropertyUtil.getStringListProperty(
            doc,
            ReponsesConstant.REPONSES_MAILBOX_SCHEMA,
            ReponsesConstant.REPONSES_MAILBOX_PRECALCUL_LIST_PROP
        );
        List<PreComptage> precalculs = new ArrayList<>();
        for (String str : pre) {
    ...
    public void setPreComptages(List<PreComptage> precomptages) {
        List<String> listPrecomptage = new ArrayList<>();
    ...
    public void decrPreComptage(String ministereId, String routingTaskType) {
        if (ministereId == null || routingTaskType == null) {
            throw new IllegalArgumentException(
                "Argument should not be null [ministereId=" + ministereId + ",routingTaskType=" + routingTaskType + "]"
            );
        }
        List<PreComptage> precomptages = getPreComptages();
        ...
        for (PreComptage precomptage : precomptages) {
            if (
                precomptage.getMinistereId().equals(ministereId) &&
                precomptage.getRoutingTaskType().equals(routingTaskType)
            ) {
