...
public class MigrationGouvernementListener implements PostCommitEventListener {
    private static final STLogger LOGGER = STLogFactory.getLog(MigrationGouvernementListener.class);
    ...
    protected String reponsesLoggingId;
    protected String newTimbreUnchangeEntity;
    protected String newTimbreDeactivateEntity;
    protected Map<String, OrganigrammeNodeTimbreDTO> mapDto;
    ...
    protected void processMigration(CoreSession session) {
        final MigrationLogger migrationLogger = MigrationLogger.getInstance();
        final Set<OrganigrammeNode> bdcToCancel = new HashSet<>();
        final Map<String, List<OrganigrammeNode>> posteBdcToMigrate = new HashMap<>();
        ...
        for (Entry<String, String> entry : newTimbre.entrySet()) {
            ...
            try {
                ...
                final DocumentModel lineDoc = createLine(session, reponsesLogging, DASH + currentMinId + DASH + newMin);
                ...
                if (newTimbreDeactivateEntity.equals(newMin)) {
                    ...
                    addLogToLine(
                        ...
                        listLog,
                        session,
                        "Fin changement de gouvernement : [" +
                        currentMinDto.getLabel() +
        ...
        // cancel bdc
        postesService.deactivateBdcPosteList(new ArrayList<>(bdcToCancel));
        // migrate bdc user
    ...
    private void runMigrationUnrestricted(final CoreSession coreSession) {
        new UnrestrictedSessionRunner(coreSession) {

            @Override
            public void run() {
                processMigration(session);
            }
        }
            .runUnrestricted();
    }
    ...
    private void deactivateEntity(
        ...
        reponsesLoggingLine.setMessage(
            "Migration de l'organigramme : désactivation du ministère [" + currentMinDto.getLabel() + CLOSED_BRACKET
        );
     ...
     *
     */
    protected void migrateOldEntityToNewGouv(
        final CoreSession session,
        final ReponsesLoggingLine reponsesLoggingLine,
        final List<String> listLog,
        final OrganigrammeNodeTimbreDTO currentMinDto
        ...
        reponsesLoggingLine.setMessage(
            MIGRATION_ORGA + currentMinDto.getLabel() + BRACKET_VERS_BRACKET + currentMinDto.getLabel() + CLOSED_BRACKET
        );
     ...
     *
     */
    private void addLogAfterMigrationEntite(
        final CoreSession session,
        final ReponsesLoggingLine reponsesLoggingLine,
        final List<String> listLog,
        final OrganigrammeNodeTimbreDTO currentMinDto
     ...
     *
     */
    protected void migrateOldEntityToNewEntity(
        final CoreSession session,
        final ReponsesLoggingLine reponsesLoggingLine,
        final List<String> listLog,
        final OrganigrammeNodeTimbreDTO currentMinDto,
        final Set<OrganigrammeNode> bdcToCancel,
        final OrganigrammeNode newMinistere,
        final ReponsesLogging reponsesLogging,
        final Map<String, List<OrganigrammeNode>> posteBdcToMigrate
    ) {
        ...
        String message =
            MIGRATION_ORGA + currentMinDto.getLabel() + BRACKET_VERS_BRACKET + newMinistere.getLabel() + CLOSED_BRACKET;
     ...
     * @param newMinistere
     */
    protected void migrateFdrImpossible(
        final ReponsesLoggingLine reponsesLoggingLine,
        final List<String> listLog,
        final OrganigrammeNode newMinistere
    ) {
        String message =
            "Migration des feuilles de route impossible. Pas de poste BDC dans le nouveau ministère : " +
            newMinistere.getLabel();
        listLog.add(message);
     ...
     *
     */
    protected void migrateOrgaAndFdr(
        final CoreSession session,
        final ReponsesLoggingLine reponsesLoggingLine,
        final List<String> listLog,
        ...
        addLogToLine(
            ...
            listLog,
            session,
            CHGT_GOUVT + currentMinDto.getLabel() + BRACKET_VERS_BRACKET + newMinistere.getLabel() + CLOSED_BRACKET,
            STLogEnumImpl.MIGRATE_MINISTERE_TEC
        );

        migrateEntiteToNewGouvernement(session, newMinistere, reponsesLoggingLine, currentMinDto);

        final Mailbox newMailboxBdc = mailboxPosteService.getOrCreateMailboxPosteNotUnrestricted(
            session,
            newPosteBdc.getId()
        );
        final String newMailboxBdcId = (String) newMailboxBdc
            .getDocument()
            .getProperty(STSchemaConstant.MAILBOX_SCHEMA, STSchemaConstant.MAILBOX_ID_PROPRETY);
        ...
        feuilleRouteModelService.migrateMinistereFeuilleRouteModel(
            session,
            currentMinDto.getId().toString(),
            newMinistere.getId().toString(),
            null,
            newMailboxBdcId,
            reponsesLoggingLine
        );
        ...
        addLogToLine(
            ...
            listLog,
            session,
            "Fin de migration des modèles de feuille de route",
            SSLogEnumImpl.MIGRATE_MOD_FDR_TEC
        );
        addLogToLine(
            reponsesLoggingLine,
            listLog,
            session,
            "Migration des dossiers impossible pas de poste bdc dans " + currentMinDto.getLabel(),
     ...
     *
     */
    protected void migrateModeleOrgaAndDossier(
        final CoreSession session,
        final ReponsesLoggingLine reponsesLoggingLine,
        final List<String> listLog,
        final OrganigrammeNodeTimbreDTO currentMinDto,
        ...
        final ReponsesLogging reponsesLogging,
        final Map<String, List<OrganigrammeNode>> posteBdcToMigrate,
        final OrganigrammeNode oldPosteBdc,
        final OrganigrammeNode newPosteBdc
    ) {
        if (posteBdcToMigrate.get(oldPosteBdc.getId()) == null) {
            posteBdcToMigrate.put(oldPosteBdc.getId(), new ArrayList<>());
        }
        ...
        final Mailbox oldMailboxBdc = mailboxPosteService.getOrCreateMailboxPosteNotUnrestricted(
            session,
            oldPosteBdc.getId()
        );
        final String oldMailboxBdcId = (String) oldMailboxBdc
            .getDocument()
            .getProperty(STSchemaConstant.MAILBOX_SCHEMA, STSchemaConstant.MAILBOX_ID_PROPRETY);
        ...
        try {
            ...
            String messageLog =
                "Erreur lors de la migration des modele de feuille de route du ministère : " + currentMinDto.getId();
        ...
        try {
            migrationService.migrateAllDossiersForReaffectation(
                session,
                currentMinDto,
                newMinistere,
                oldMailboxBdcId,
        ...
        if (currentMinDto.getMigratingDossiersClos()) {
            addLogToLine(
                ...
                listLog,
                session,
                "Migration des questions closes",
                STLogEnumImpl.MIGRATE_DOSSIER_TEC
            );
            try {
                migrationService.migrateDossierClos(
                    session,
                    currentMinDto.getId(),
                    newMinistere,
                    reponsesLoggingLine,
                ...
                String messageLog =
                    "Erreur lors de la migration des dossiers clos du ministère : " + currentMinDto.getId();
                logMigrationFailure(
                    session,
                    reponsesLoggingLine,
                    STLogEnumImpl.FAIL_MIGRATE_DOSSIER_TEC,
            ...
            addLogToLine(
                ...
                listLog,
                session,
                "Fin de migration des questions closes",
                STLogEnumImpl.MIGRATE_DOSSIER_TEC
            );
            reponsesLogging.save(session);
        }
     ...
     * @param exc
     */
    private void logMigrationFailure(
        final CoreSession session,
        final ReponsesLoggingLine reponsesLoggingLine,
        final STLogEnum logEnum,
     ...
     *
     */
    protected void migrateEntiteToNewGouvernement(
        final CoreSession session,
        final OrganigrammeNode newMinistere,
        final ReponsesLoggingLine reponsesLoggingLine,
        final OrganigrammeNodeTimbreDTO currentMinDto
        ...
        if (StringUtils.isNotBlank(detailBriserSignature)) {
        ...
        reponsesLoggingLine.save(session);
        reponsesLoggingLineDetail.setMessage(
            MIGRATION_ORGA + currentMinDto.getLabel() + BRACKET_VERS_BRACKET + newMinistere.getLabel() + CLOSED_BRACKET
        );
        ...
        try {
            String message =
                "Début de la migration de l'organigramme [" +
                currentMinDto.getLabel() +
                BRACKET_VERS_BRACKET +
                newMinistere.getLabel() +
                CLOSED_BRACKET;
            ...
            message =
                "Fin de la migration de l'organigramme [" +
                currentMinDto.getLabel() +
                BRACKET_VERS_BRACKET +
                newMinistere.getLabel() +
                CLOSED_BRACKET;
     ...
     * @return
     */
    protected String briseMinistereSignature(
        final CoreSession session,
        final ReponsesLoggingLine reponsesLoggingLine,
        final OrganigrammeNodeTimbreDTO currentMinDto
    ) {
        ...
        if (currentMinDto.getBreakingSeal()) {
            try {
                ...
                List<DocumentModel> lstDossiersABriser = updateTimbreService.getSignedDossiersForMinistere(
                    session,
                    currentMinDto.getId().toString()
                );
                ...
                if (lstDossiersABriser.isEmpty()) {
                    listLog.add(
                        "Brisure du cachet serveur pour les questions du timbre : " +
                        currentMinDto.getLabel() +
                    ...
                    listLog.add(
                        ...
                        currentMinDto.getLabel() +
                        ", " +
                        lstDossiersABriser.size() +
                        " dossier(s) impacté(s)"
                    );
                    List<String> lstDossiersEnErreur = new ArrayList<>();
                    List<String> lstDossierTraites = migrationService.briserSignatureDossiers(
                        session,
                        lstDossiersABriser,
                        lstDossiersEnErreur
                    );
                    listLog.add(
                        "Liste des dossiers traités lors de la brisure du cachet : " +
                        StringUtils.join(lstDossierTraites, ";", "'")
                    );

                    // S'il reste des dossiers à traiter après le traitement on marque le traitement en erreur
                    if (!lstDossiersEnErreur.isEmpty()) {
                        String dossierConcat = StringUtils.join(lstDossiersEnErreur, ";", "'");
                        listLog.add(
                            "Erreur de brisure du cachet des dossiers, les dossiers suivants n'ont PAS été traités : " +
                            dossierConcat
                        );
                ...
                MigrationLogger
                    .getInstance()
                    .logErrorMigration(
                        SSLogEnumImpl.FAIL_MIGRATE_FDR_TEC,
                        "Une erreur est survenue lors de la brisure de la signature ",
                        ex
                    );
