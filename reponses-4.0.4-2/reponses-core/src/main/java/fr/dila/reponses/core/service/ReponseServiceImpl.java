...
public class ReponseServiceImpl implements ReponseService {
    ...
    @Override
    public DocumentModel saveReponse(
        CoreSession session,
        final DocumentModel inputReponseDoc,
        ...
        if (dossier != null) {
            ...
            final DocumentEventContext journalEventContextReponse = new DocumentEventContext(
                session,
    ...
    @Override
    public DocumentModel saveReponseAndErratum(
        CoreSession documentManager,
        DocumentModel reponseDoc,
    ...
    public DocumentModel briserSignatureReponse(CoreSession session, Reponse reponse, DocumentModel dossier) {
        ...
        if (dossierObject != null) {
            if (allotissementService.isAllotit(dossierObject)) {
    ...
    @Override
    public DocumentModel saveReponseFromMinistere(CoreSession session, DocumentModel reponse) {
        ...
        producer.fireEvent(
            journalEventContextReponse.newEvent(ReponsesEventConstant.DOSSIER_REPONSE_VERSION_UPDATE_EVENT)
        );
     ...
     */
    @Override
    public DocumentModel getDossierFromReponse(CoreSession session, DocumentModel reponse) {
    ...
    public List<String> getVersionsContributorsFromReponse(CoreSession session, DocumentModel reponse) {
        List<String> versionContributors = new ArrayList<>();
        for (DocumentModel doc : getReponseVersionDocumentList(session, reponse)) {
            versionContributors.add(
    ...
    @Override
    public DocumentModel incrementReponseVersion(CoreSession session, DocumentModel reponse) {
        // Increment major version number
        reponse.putContextData(VersioningService.VERSIONING_OPTION, VersioningOption.MAJOR);
    ...
    @Override
    public int getReponseMajorVersionNumber(CoreSession session, DocumentModel reponse) {
        final Long majorVersion = (Long) reponse.getProperty(
