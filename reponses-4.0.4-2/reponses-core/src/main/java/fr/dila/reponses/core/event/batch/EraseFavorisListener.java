 ...
 * @author lbosse
 */
public class EraseFavorisListener extends AbstractBatchEventListener implements StoppableBatch {
    /**
    ...
    protected void processEvent(final CoreSession session, final Event event) {
        ...
        try {
            suiviBatchService.createBatchResultFor(
                batchLoggerModel,
                "Suppression des favoris arrivés à expiration",
                endTime - startTime
            );
        } catch (Exception e) {
    ...
    public STLogEnum getStoppageCode() {
        return ReponsesLogEnumImpl.CANCEL_B_DEL_FAV_TEC;
    }
