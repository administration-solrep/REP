 ...
 *
 */
public class ExtractionQuestionsListener extends AbstractBatchEventListener implements StoppableBatch {
    ...
    protected void processEvent(final CoreSession session, final Event event) {
        ...
        if (!QuestionTypesEnum.isValueInEnum(type)) {
            StringBuilder message = new StringBuilder(
                "Le paramètre QTYPE doit être égal à l'une des valeurs suivantes : "
            );
        ...
        if (source == null || (!"SENAT".equals(source) && !"AN".equals(source))) {
            LOGGER.error(
                session,
                ReponsesLogEnumImpl.PROCESS_B_EXTRACTION_QUESTIONS_TEC,
                "Le paramètre QSOURCE doit être égal à l'une des valeurs suivantes : SENAT ou AN"
            );
            return;
        }
        ...
        if ("OUVERTES".equals(statut)) {
            ...
            LOGGER.error(
                session,
                ReponsesLogEnumImpl.PROCESS_B_EXTRACTION_QUESTIONS_TEC,
                "Le paramètre QSTATUT doit être égal à OUVERTES ou FERMEES"
            );
            return;
        }
     ...
     * @return
     */
    private ExtractionQuestionsResponse generateXML(
        final List<Question> result,
        final String type,
        final String source,
        final StatutsEnum statutEnum
    ) {
        ...
        for (Question question : result) {
            ...
            questionXML.setDate_publication_jo(
                SolonDateConverter.DATE_DASH_REVERSE.format(question.getDatePublicationJO())
            );
            ...
            if (question.getDateRenouvellementQuestion() != null) {
                questionXML.setDate_renouvellement_question(
                    SolonDateConverter.DATE_DASH_REVERSE.format(question.getDateRenouvellementQuestion())
                );
            }
            if (question.getDateSignalementQuestion() != null) {
                questionXML.setDate_signalement_question(
                    SolonDateConverter.DATE_DASH_REVERSE.format(question.getDateSignalementQuestion())
                );
            }
            ...
            if (dateCloture != null) {
                questionXML.setDate_abandon_question(SolonDateConverter.DATE_DASH_REVERSE.format(dateCloture));
            } else if (dateCaducite != null) {
                questionXML.setDate_abandon_question(SolonDateConverter.DATE_DASH_REVERSE.format(dateCaducite));
            } else if (dateRetrait != null) {
                questionXML.setDate_abandon_question(SolonDateConverter.DATE_DASH_REVERSE.format(dateRetrait));
            }
     ...
     * @param statutEnum statut des questions
     */
    private void marshallFile(
        final CoreSession session,
        final ExtractionQuestionsResponse xmlResult,
        final String source,
        final StatutsEnum statutEnum
    ) {
        try {
            ...
            String dirPath = STServiceLocator
                .getSTParametreService()
                .getParametreValue(session, STParametreConstant.EXTRACTION_REPERTOIRE);
            ...
            StringBuilder filePath = new StringBuilder(source)
                ...
                .append(statutStr)
                .append("_")
                .append(SolonDateConverter.DATETIME_DASH_REVERSE_SECOND_NOSEP.formatNow())
                .append(".xml");
    ...
    public STLogEnum getStoppageCode() {
        return ReponsesLogEnumImpl.CANCEL_B_EXTRACTION_QUESTIONS_TEC;
    }
