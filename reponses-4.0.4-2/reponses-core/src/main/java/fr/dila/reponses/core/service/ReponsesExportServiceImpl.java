...
import static fr.dila.ss.api.constant.SSBirtConstants.BIRT_REPORT_ROOT;
...
public class ReponsesExportServiceImpl extends STExportServiceImpl implements ReponsesExportService {
    ...
    private static final STLogger LOGGER = STLogFactory.getLog(ReponsesExportServiceImpl.class);

    private static final Map<String, String> correspNomsStats = Collections.unmodifiableMap(initNomsStats());

    private static Map<String, String> initNomsStats() {
        Map<String, String> nomsStats = new HashMap<>();
            ...
            if (exportStatsFolderRoot != null) {
                exportStatDoc =
                    session.createDocumentModel(
                        exportStatsFolderRoot,
                        EXPORT_STAT_DOCUMENT_NAME,
    ...
    @Override
    public String getOrCreateExportStatRootPath(CoreSession session, String userWorkspacePath) {
        return this.getOrCreateExportDocumentRootPath(
                session,
                userWorkspacePath,
                EXPORT_STAT_FOLDER_ID,
    ...
    public DocumentModel getOrCreateExportStatRootDoc(CoreSession session, String userWorkspacePath) {
        return this.getOrCreateExportDocumentRootDoc(
                session,
                userWorkspacePath,
                EXPORT_STAT_FOLDER_ID,
                EXPORT_STAT_FOLDER_TYPE
    ...
    public void exportStat(
        ...
        if (exportStatDoc != null) {
            ...
            if (reportsMultiExportMap.isEmpty() || reportsNamesMap.isEmpty() || reportsTitlesMap.isEmpty()) {
                throw new ReponsesException(SSExceptionConstants.EMPTY_REPORTS_EXC);
            } else {
                if (
                    reportsMultiExportMap.size() != reportsNamesMap.size() ||
                    reportsNamesMap.size() != reportsTitlesMap.size()
                ) {
                    throw new ReponsesException(SSExceptionConstants.DIFFERENTS_SIZES_REPORTS_EXC);
                }
                ...
                Blob zipBlob = BlobUtils.createSerializableBlob(
                    zipStream,
                    name + STExportConstants.EXPORT_ZIP_EXTENSION,
                    null
                );
                zipStream.close();
    ...
    private String generateNameForZip(ExportDocument exportStat, Map<String, String> reportsTitlesMap) {
        ...
        if (dateRequest == null) {
            dateRequestStr = SolonDateConverter.DATE_DASH_REVERSE.formatNow();
        } else {
            dateRequestStr = SolonDateConverter.DATE_DASH_REVERSE.format(dateRequest);
        }

        if (reportsTitlesMap.size() > 1) {
            nameSb.append("REP-STATS");
        } else if (reportsTitlesMap.size() == 1) {
            String title = StringHelper.removeSpacesAndAccent((String) reportsTitlesMap.values().toArray()[0]);
    ...
    /**
     * Parcourt la map des rapports à exporter et génère les stats correspondantes
     * dans les formats indiqués et pour tous les ministères ou directions en cours
     * si le rapport est multi-ministère ou multi-direction
     *
     * @param zipFile
     ...
     * @throws IOException
     */
    protected void exportStatAndZipIt(
        File zipFile,
        Map<String, String> reportsMultiExportMap,
        Map<String, String> reportsNamesMap,
        Map<String, String> reportsTitlesMap,
        ...
        List<UniteStructurelleNode> directionsList = new ArrayList<>();
        Map<String, Serializable> inputValues = null;

        final StringBuilder fileName = new StringBuilder();
        Map<String, Serializable> emptyHashMap = new HashMap<>();
        for (Entry<String, String> entry : reportsMultiExportMap.entrySet()) {
            ...
            if (entry.getValue().equals("MIN")) {
                ...
                for (OrganigrammeNode ministereNode : ministeresList) {
                    inputValues.put("parameterValue", ministereNode.getId());
                    ...
                    reports.addAll(
                        birtService
                            .generateReportFileResults(
                                StringHelper.removeSpacesAndAccent(name),
                                BIRT_REPORT_ROOT + name,
                                inputValues,
                                formats
                            )
                            .values()
    ...
    @Override
    public int removeOldExportStat(CoreSession session, Calendar dateLimit) {
    ...
    private List<UniteStructurelleNode> extractSubUnitesStructurellesRecursive(
        List<UniteStructurelleNode> uniteStructurelles
    ) {
        List<UniteStructurelleNode> usList = new ArrayList<>();
        List<UniteStructurelleNode> tmpList;
