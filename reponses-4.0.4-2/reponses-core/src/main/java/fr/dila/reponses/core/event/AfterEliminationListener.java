...
public class AfterEliminationListener implements PostCommitEventListener {
    ...
    public void handleEvent(EventBundle events) {
        for (Event event : events) {
            if (
                event.getName().equals(ReponsesEventConstant.AFTER_ELIMINATION_LISTE) ||
    ...
    private final void handleEvent(Event event) {
        ...
        final DocumentModel eliminationList = (DocumentModel) ctx.getProperty(
            ReponsesEventConstant.DOSSIER_EVENT_PARAM
        );
