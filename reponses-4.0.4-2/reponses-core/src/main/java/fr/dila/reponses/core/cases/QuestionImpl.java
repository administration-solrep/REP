...
public class QuestionImpl extends STDomainObjectImpl implements Question {
    ...
    private static final String QUERY_ETAT_QUESTION =
    ...
    @Override
    public DocumentModel getQuestionMetadata(Question question) {
        /**
    ...
    protected List<QErratum> getErrataProperty(String schema, String value) {
        ...
        List<Map<String, Serializable>> errata = PropertyUtil.getMapStringSerializableListProperty(
            getDocument(),
            schema,
            value
        );
    ...
    protected List<QuestionStateChange> getEtatsQuestionsDoc(CoreSession session) {
        List<QuestionStateChange> questionStateChangeList = new ArrayList<>();
        try {
            List<DocumentModel> etatQuestionDocs = QueryUtils.doUFNXQLQueryAndFetchForDocuments(
                session,
                "EtatQuestion",
                QUERY_ETAT_QUESTION,
    ...
    public void setErrata(List<QErratum> errata) {
        ArrayList<Map<String, Serializable>> listeErratum = new ArrayList<>();
        for (QErratum erratum : errata) {
            Map<String, Serializable> erratumMap = new HashMap<>();
    ...
    public void setNumeroQuestion(Long numeroQuestion) {
        setProperty(
            DossierConstants.QUESTION_DOCUMENT_SCHEMA,
    ...
    private void addHistoriqueEtatQuestion(CoreSession session, Calendar date, String label) {
        String titreDoc = "EtatQuestion-" + SolonDateConverter.DATE_DASH_REVERSE.format(date) + "-" + label;

        try {
            DocumentModel etatDoc = session.createDocumentModel(
    ...
    protected static boolean shouldBeListedInHistoric(String etatQuestion) {
        return (
            canBeCurrentStateOfQuestion(etatQuestion) ||
    ...
    public String getIdMandat() {
        return getStringProperty(
            DossierConstants.QUESTION_DOCUMENT_SCHEMA,
            DossierConstants.DOSSIER_ID_MANDAT_QUESTION
        );
    }
    ...
    public void setSenatQuestionId(String senatQuestionId) {
        setProperty(
            DossierConstants.QUESTION_DOCUMENT_SCHEMA,
            DossierConstants.DOSSIER_SENAT_ID_QUESTION,
            senatQuestionId
        );
    }
    ...
    @Override
    public Calendar getDateClotureQuestion() {
        return getDateProperty(
            DossierConstants.QUESTION_DOCUMENT_SCHEMA,
            DossierConstants.DOSSIER_DATE_CLOTURE_QUESTION
        );
    }
    ...
    public Calendar getDateRappelQuestion() {
        return getDateProperty(
            DossierConstants.QUESTION_DOCUMENT_SCHEMA,
            DossierConstants.DOSSIER_DATE_RAPPEL_QUESTION
        );
    }
    ...
    public List<Signalement> getSignalements() {
        return getSignalementsProperty(
            DossierConstants.QUESTION_DOCUMENT_SCHEMA,
            DossierConstants.DOSSIER_SIGNALEMENTS
        );
    }

    @Override
    public void setSignalements(List<Signalement> signalements) {
        ArrayList<Map<String, Serializable>> listeSignalement = new ArrayList<>();
    ...
    public List<Renouvellement> getRenouvellements() {
        return getRenouvellementsProperty(
            DossierConstants.QUESTION_DOCUMENT_SCHEMA,
    ...
    private void setRenouvellements(List<Renouvellement> renouvellements) {
        List<Map<String, Serializable>> listeRenouvellement = new ArrayList<>();
    ...
    public Boolean hasIndexationSenat() {
        return (
            !getSenatQuestionRenvois().isEmpty() ||
            !getSenatQuestionThemes().isEmpty() ||
    ...
    public Boolean hasIndexationComplementaireAn() {
        return (
            !getIndexationComplAssNatAnalyses().isEmpty() ||
            !getIndexationComplAssNatRubrique().isEmpty() ||
    ...
    public Boolean hasIndexationComplementaireSenat() {
        return (
            !getIndexationComplSenatQuestionRubrique().isEmpty() ||
    ...
    public String getIntituleMinistereRattachement() {
        return getStringProperty(
            DossierConstants.QUESTION_DOCUMENT_SCHEMA,
            DossierConstants.DOSSIER_INTITULE_MINISTERE_RATTACHEMENT_QUESTION
        );
    }
    ...
    @Override
    public Optional<Calendar> getDateReponseSignalement() {
        return getSignalements()
            .stream()
            .filter(s -> Objects.equals(s.getDateEffet(), getDateSignalementQuestion().getTime()))
            .findFirst()
            .map(s -> DateUtil.toCalendarFromNotNullDate(s.getDateAttendue()));
    }

    @Override
    public String getCiviliteEtNomCompletAuteur() {
        return getCiviliteAuteur() + " " + getNomCompletAuteur();
    }
}
