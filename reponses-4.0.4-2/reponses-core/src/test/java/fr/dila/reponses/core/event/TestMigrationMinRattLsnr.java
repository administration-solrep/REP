...
public class TestMigrationMinRattLsnr {
    @Inject
    protected MigrationMinistereRattachementListener listener;
    ...
    public void testMigrateEntiteOK() {
        ...
        listener.migrateMin(session, loggerDoc.getId(), timbreMap, "unchanged_entity", "deactivate_entity");
    ...
    public void testMigrateEntiteFail() {
        ...
        Mockito.doThrow(Exception.class).when(updateTimbreService).getCloseCountForMinistere(session, "2");
    ...
    public void assertMigrationOKResponse(List<String> lstLogLines) {
        ...
        assertEquals(2, lstLogLines.size());
        ...
        assertEquals(2, logLine1.getFullLog().size());
        assertEquals("Migration des ministeres de rattachement Questions Closes", logLine1.getFullLog().get(0));
        assertEquals("Fin de Migration des ministeres de rattachement Questions Closes", logLine1.getFullLog().get(1));
    }

    public void assertMigrationFailResponse(List<String> lstLogLines) {
        assertNotNull(lstLogLines);
        assertEquals(1, lstLogLines.size());
        DocumentModel doc1 = session.getDocument(new IdRef(lstLogLines.get(0)));
        ReponsesLoggingLine logLine1 = doc1.getAdapter(ReponsesLoggingLine.class);
        assertEquals(
            "Migration de l'organigramme Questions Closes: [Migration ministère 1] vers [Migration ministère 1]",
            logLine1.getMessage()
        );
        assertEquals(ReponsesLoggingStatusEnum.SUCCESS, logLine1.getStatus());
    }
    ...
    private void assertAllMinistereMigrated(List<String> lstLogLines) {
        ...
        assertEquals(
            "Migration de l'organigramme Questions Closes: [Migration ministère 2] vers [Migration ministère 2]",
            logLine2.getMessage()
        );
    }
}
